...
export const Route = createFileRoute("/_unregistered/demo/shiftboard")({
  head: () => ({
    ...
    // 候補A: 無料デモ｜店長・シフト担当者視点で操作できるシフト管理サンドボックス（伝わりにくい）
    // 候補B: 資料請求なしで触れる｜飲食店向けシフト管理シフトリのデモ（差別化明確だが検索数小）
    links: buildLinks({ canonical: "/demo/shiftboard" }),
    meta: buildMeta({
      title: "登録なしで試せる無料デモ｜店長・シフト担当者視点でシフト管理を体験",
      description:
        "シフトリの店長・シフト担当者画面を会員登録なしで試せる無料デモです。希望の集約から確定通知までブラウザで2分で体験できます。資料請求も商談も不要、すぐ触れます。",
