...
type Props = {
  ...
  activeShops: AppHomeShopOption[] | null;
  requestedShopId?: string;
};

export function AppHomeRoutePage({
  organizationId,
  organizationName,
  memberStatus,
  activeShops,
  ...
  const preferredShopId = readAppHomeShopPreference(storage, organizationId);
  const resolution = resolveAppHomeShop(activeShops, requestedShopId, preferredShopId);
      ...
      {resolution.kind === "empty" ? (
        ...
        <Animation>
          {resolution.kind === "loading" ? (
            <AppHomePageStateView state={{ kind: "loading" }} />
          ) : (
            <ErrorBoundary
              key={`${organizationId}:${resolution.shop.id}`}
              fallback={<AppHomePageStateView state={{ kind: "error" }} onReload={() => window.location.reload()} />}
            >
...
function ConnectedAppHome({
  ...
  const shopContexts = useMemo(
    () =>
      buildAppHomeShopContexts(activeShops, {
        id: organizationId,
  ...
  if (shop === undefined || currentUser === undefined || managerLegalConsentStatus === undefined) {
    return <AppHomePageStateView state={{ kind: "loading" }} />;
  }

  if (shop === null || !selectedShop) {
    return <AppHomePageStateView state={{ kind: "inaccessible" }} onReload={() => window.location.reload()} />;
  }
...
export function AppHomeReadOnlyNotice({
  organizationId,
  memberStatus,
...
export type AppHomePageState = { kind: "loading" } | { kind: "empty" } | { kind: "inaccessible" } | { kind: "error" };

export function AppHomePageStateView({
  state,
  onOpenManagement,
  onReload,
}: {
  state: AppHomePageState;
