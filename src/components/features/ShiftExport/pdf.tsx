...
export const createShiftPdf = async (schedule: ExportSchedule): Promise<Blob> => {
  ...
  return pdf(
    <Document title={getExportTitle(schedule)} author="シフトリ" language="ja">
      {pages.map(({ period, layout, rows: staffs, isFirstPage }, pageIndex) => {
        const cellStyle = {
        ...
        const title = fitExportText(getExportTitle(period), layout.pageWidthPt - layout.marginPt * 2, 16);
        return (
          <Page
            key={`${period.periodStart}:${staffs[0]?.staffId ?? "empty"}`}
            size="A4"
