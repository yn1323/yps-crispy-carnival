...
describe("店舗詳細の基本情報", () => {
  it("時間指定の勤務時間と定休日を読み取り用の行へ整形する", () => {
    expect(
      ...
      { label: "店舗名", value: "渋谷店" },
      { label: "提出方法", value: "時間指定" },
      { label: "勤務時間", value: "09:00〜翌1:00" },
      { label: "定休日", value: "毎週 金・日" },
    ]);
  });

  it("日付選択は勤務時間の行を出さず、定休日なしと表示する", () => {
    expect(
      getShopBasicInformationRows({
        name: "新宿店",
      ...
      { label: "店舗名", value: "新宿店" },
      { label: "提出方法", value: "日付選択" },
      { label: "定休日", value: "定休日なし" },
    ]);
  });

  it("勤務パターンはすべてのパターン名と時間を表示する", () => {
    expect(
      getShopBasicInformationRows({
        name: "池袋店",
        submissionPattern: {
      ...
      { label: "店舗名", value: "池袋店" },
      { label: "提出方法", value: "パターン選択" },
      { label: "勤務パターン", value: "早番（09:00〜15:00）\n遅番（15:00〜21:00）" },
