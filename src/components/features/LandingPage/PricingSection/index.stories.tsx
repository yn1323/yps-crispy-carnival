...
export const Desktop: Story = {
  play: async ({ canvasElement }) => {
    ...
    const trialNotice = within(canvas.getByLabelText("Businessプランの無料トライアル"));

    await expect(trialNotice.getByText("登録から3か月間")).toBeInTheDocument();
    await expect(trialNotice.getByText("Businessプランを無料でお試し")).toBeInTheDocument();
    await expect(trialNotice.getByText("クレジットカード不要")).toBeInTheDocument();
    await expect(trialNotice.getByText("スタッフ40名・5店舗・管理者5名まで")).toBeInTheDocument();
  },
};
