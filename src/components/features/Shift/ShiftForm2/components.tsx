...
export const Avatar = ({ staff, size = 28 }: { staff: Staff; size?: number }) => (
  <Box
    style={{
      width: size,
      height: size,
...
export const SaveButton = ({ compact = false }: { compact?: boolean }) => (
  <button
    type="button"
    aria-label="一時保存"
    style={{
...
export const ConfirmButton = ({ compact = false }: { compact?: boolean }) => (
  <button
    type="button"
    style={{
      height: compact ? 28 : 32,
  ...
  const items = [
    {
      ...
      label: "印刷",
      sub: "バックヤードに貼り出し",
      icon: <LuPrinter size={16} color="#0f766e" />,
    },
    {
      k: "csv",
      label: "CSVで書き出し",
      sub: "給与計算ソフト向け",
      icon: <LuFileText size={16} color="#0f766e" />,
    },
    {
      k: "excel",
      label: "Excelで書き出し",
