...
import type { AIInput, PatternType, PositionType, StaffingEntry } from "../types";
...
export const SetupWizard = ({ openTime, closeTime, positions, onSave, onCancel }: SetupWizardProps) => {
  ...
  const handleAddPattern = () => {
    ...
    const availableDays = [0, 1, 2, 3, 4, 5, 6, 7].filter((d) => !usedDays.includes(d));
  ...
  // 全曜日が設定済みかどうか
  const allDaysAssigned = patterns.flatMap((p) => p.appliedDays).length === DAY_COUNT;
