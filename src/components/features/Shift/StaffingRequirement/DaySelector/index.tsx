...
export const DaySelector = ({
  ...
  const selectWeekends = () => {
    const weekends = [0, 6, 7].filter((d) => !disabledDays.includes(d));
    onChange(weekends);
  };

  // 全選択
  const selectAll = () => {
    const allDays = [0, 1, 2, 3, 4, 5, 6, 7].filter((d) => !disabledDays.includes(d));
    onChange(allDays);
  };
      ...
      <Flex gap={{ base: 2, md: 4 }} mt={2}>
        ...
        {[6, 0, 7].map((dayIndex) => (
          ...
          >
            <Checkbox.HiddenInput />
            <Checkbox.Control />
            <Checkbox.Label color={getDayColor(dayIndex)}>{DAY_LABELS[dayIndex]}</Checkbox.Label>
          </Checkbox.Root>
        ))}
      </Flex>
    </Box>
