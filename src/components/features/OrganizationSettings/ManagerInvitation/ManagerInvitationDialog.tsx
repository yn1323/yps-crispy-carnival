...
export function ManagerInvitationDialog({
  ...
  const shouldRestoreConfirmationTriggerRef = useRef(false);

  useEffect(() => {
    setIsConfirmingFreeManagerExchange(false);
    shouldRestoreConfirmationTriggerRef.current = false;
    if (isOpen) {
  ...
  const confirmationProps =
    isShowingFreeManagerExchangeConfirmation && selectedStaff
      ? {
          personName: selectedStaff.name,
          personEmail: selectedStaff.email,
          mode: "freeManagerExchange" as const,
          isResend: selectedStaff.isResend,
        }
      : null;
  const confirmationCopy = confirmationProps ? getManagerAssignmentConfirmationCopy(confirmationProps) : null;
  ...
  useEffect(() => {
    if (isShowingFreeManagerExchangeConfirmation) {
      confirmationBodyRef.current?.focus();
      return;
    }
    if (!shouldRestoreConfirmationTriggerRef.current) return;
    shouldRestoreConfirmationTriggerRef.current = false;
    normalFormRef.current
      ?.closest('[role="dialog"]')
      ?.querySelector<HTMLButtonElement>('button[form="invite-manager-form"]')
      ?.focus();
  ...
  const leaveConfirmation = () => {
    if (!isShowingFreeManagerExchangeConfirmation || isRunning) return;
    shouldRestoreConfirmationTriggerRef.current = true;
    setIsConfirmingFreeManagerExchange(false);
  };
  const closeDialog = () => {
    shouldRestoreConfirmationTriggerRef.current = false;
  ...
  return (
    <Dialog
      title={
        ...
        (isResendOnly ? "ログイン案内を再送" : isFreeManagerExchange ? "次の管理者を招待" : "新しい管理者を招待")
      }
      role={isShowingFreeManagerExchangeConfirmation ? "alertdialog" : "dialog"}
      isOpen
      onOpenChange={handleOpenChange}
      onClose={isShowingFreeManagerExchangeConfirmation ? leaveConfirmation : closeDialog}
      closeLabel={isShowingFreeManagerExchangeConfirmation ? "キャンセル" : undefined}
      formId={isShowingFreeManagerExchangeConfirmation ? undefined : "invite-manager-form"}
      onSubmit={
        confirmationProps && selectedStaff ? () => onSubmit({ kind: "person", personId: selectedStaff.id }) : undefined
      }
      submitLabel={confirmationCopy?.confirmLabel ?? submitLabel}
      isLoading={isRunning}
      isSubmitDisabled={!isShowingFreeManagerExchangeConfirmation && !canSubmit}
      mobileActionLayout="stacked"
      mobileFullScreen
      maxW={{ lg: "520px" }}
      maxH={{ lg: "min(760px, calc(100dvh - 48px))" }}
    >
      {confirmationProps ? (
        <Stack
          ref={confirmationBodyRef}
          data-testid="manager-invitation-confirmation-body"
          tabIndex={-1}
          outline="none"
        >
          <ManagerAssignmentConfirmation {...confirmationProps} />
        </Stack>
      ) : (
        <form
          ref={normalFormRef}
