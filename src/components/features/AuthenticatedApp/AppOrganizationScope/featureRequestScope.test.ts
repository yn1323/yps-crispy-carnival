...
describe("resolveAppFeatureRequestScope", () => {
  it("Homeのshopがactive組織店舗なら既定店舗にする", () => {
    expect(resolveAppFeatureRequestScope({ pathname: "/dashboard", homeShopId: "shop-b", shops: shops })).toEqual({
      kind: "shop",
      shop: shops[1],
    });
    expect(resolveAppFeatureRequestScope({ pathname: "/dashboard/", homeShopId: "shop-b", shops: shops })).toEqual({
  ...
  it("組織外または無効なHome店舗を既定にせず、組織scopeへ戻す", () => {
    expect(resolveAppFeatureRequestScope({ pathname: "/dashboard", homeShopId: "other-shop", shops: shops })).toEqual({
      kind: "organization",
    });
  });

  it("店舗詳細はroute paramとactive店舗の一致を内部送信先にする", () => {
    expect(resolveAppFeatureRequestScope({ pathname: "/manage/shops/shop-a", shops: shops })).toEqual({
      kind: "shop",
      shop: shops[0],
    });
    expect(resolveAppFeatureRequestScope({ pathname: "/manage/shops/shop-a/", shops: shops })).toEqual({
    ...
    expect(resolveAppFeatureRequestScope({ pathname: "/shifts", shops: shops })).toEqual({
      kind: "organization",
    });
  });
  ...
  it.each(["/staff/register", "/shifts/submit"])("公開route %s は店舗scopeとして扱わない", (pathname) => {
    expect(resolveAppFeatureRequestScope({ pathname, shops: shops })).toEqual({ kind: "organization" });
  });
