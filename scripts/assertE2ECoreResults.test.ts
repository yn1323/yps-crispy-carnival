...
describe("E2E core result gate", () => {
  it("desktop 8件、mobile 1件の9契約を固定する", () => {
    const projects = [...EXPECTED_CORE_CONTRACTS.values()];

    expect(EXPECTED_CORE_CONTRACTS.size).toBe(9);
    expect(projects.filter((projectName) => projectName === "desktop-chromium")).toHaveLength(8);
