...
> 理由: `superseded`
>
> 後継: [Analytics運用](../../manual/analytics-rollout.md)

> 文書種別: manual
>
> 現在の実環境状態: [リリース状態](../../manual/release-status.md)
...
missing、期限超過、terminal failure、redaction接近の各test alertを一回ずつ送信し、通知先、owner、event IDまたは画面証跡を[リリース状態](../../manual/release-status.md)へ記録します。
...
Analytics UIとWorkerを同じ対象revisionで配信し、[セキュリティ再検証](../../manual/security-validation.md)の次を確認します。
...
実環境の結果は[リリース状態](../../manual/release-status.md)へ、次を一組として記録します。
