...
| 計画 | 状態 | 次に必要な判断 | 現在の正本 |
|---|---|---|---|
| [SPボトムナビゲーションと画面遷移](2026-08-14_SPボトムナビゲーションと画面遷移_実装計画.md) | `reviewing` | 5タブ構成、`/app/*`への段階移行、「対応」の初期対象を実装対象として承認する | 現行仕様は[機能インデックス](../features/INDEX.md)、設計原則は[UI設計方針](../rules/ui-design.md) |
