...
| 計画 | 状態 | 未完了条件 | 現在の正本 |
|---|---|---|---|
| [認証済み新ページ正式切替と旧ページ削除](2026-08-15_認証済み新ページ正式切替と旧ページ削除_実装計画.md) | `pull request verification` | repository実装、現在仕様文書、型検査、push前限定testは完了。目的別commit、現在branchのpush、Pull Request作成、最新head SHAの全test・build・全E2E・VRT captureとcompare開始の確認 | 現行仕様は[機能インデックス](../features/INDEX.md)、設計と検証は[フロントエンド方針](../rules/frontend-architecture.md)、[テスト方針](../rules/testing-strategy.md)、[セキュリティ方針](../rules/security-strategy.md) |
