...
- 状態: `Active / implementation planned`
- 作成日: 2026-08-13
...
- 実装状態: 仕様と実装順を確定。コード、テスト、deploy、Production事前確認、必要な場合のデータ変換は未実施

この計画は、LINE連携の正本を店舗スタッフからグループ内の人物へ移す。
現在仕様の正本は、実装完了まで[LINE通知連携](../features/line-notification.md)を含む`doc/features/`に置く。
...
| 旧`staffLineAccounts`を新しい正本へ移すデータ変換 | 条件付き。canonical counterpartがないactive行が0件なら実装・実行ともskip |
...
- 明示解除確認: `この組織のすべての所属店舗でLINE通知が停止します。ほかの組織のLINE連携には影響しません。`
