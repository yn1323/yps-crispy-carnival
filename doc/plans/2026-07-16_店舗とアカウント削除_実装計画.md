...
> この計画はClerkユーザー削除を前提としており、2026-07-18の要件と一致しない。
> [店舗と組織の削除、主要マスタの直接識別子置換の実装計画](2026-07-18_店舗と組織の削除_個人情報匿名化_実装計画.md)を後継計画とする。
...
また、一人の管理者が複数のグループと店舗へ所属できるため、一店舗の削除にClerkユーザー削除を無条件で結び付けると、残すべきログイン権限まで失う。

最初の実装では、日常的な店舗整理には既存の「アーカイブ」を使い、不可逆操作を「グループとアカウントを削除」として別に追加する。

この不可逆操作は、現在の管理者が唯一のグループ、唯一の管理者、唯一の未削除店舗を持つ場合だけ許可する。

条件を満たさない場合はClerkユーザーを削除せず、店舗のアーカイブ、管理者の整理、または別グループからの退出を案内する。
...
| 領域 | 現在の状態 | 根拠 |
| --- | --- | --- |
| 新規店舗 | 初期設定でグループ、管理者所属、`organizationId` 付き店舗を作る | `convex/setup/mutations.ts` |
...
| 複数所属 | `getMyShops` は複数グループの所属店舗を返す | `convex/dashboard/queries.ts` |
...
設定画面の店舗タブに、既存のアーカイブとは分離した「グループとアカウントの削除」領域を追加する。

この操作は次の処理を一つの利用終了フローとして受け付ける。

- 対象グループと唯一の店舗を利用不可にする。
...
- 対象環境の `CLERK_SECRET_KEY`、`VITE_CLERK_PUBLISHABLE_KEY`、`CLERK_JWT_ISSUER_DOMAIN` がConvexへ設定されている。
- 操作者がClerk認証済みで、ローカルの有効な管理者である。
- 認証identityのissuerと `users.authTokenIdentifier` が、対象環境のClerk利用者として一致する。
- 対象グループが削除済みではない。
- 対象グループに未削除の店舗が一件だけ存在する。
- 対象グループに、操作者以外の `active` または `readOnly` 管理者が存在しない。
- 対象の唯一店舗に、操作者以外の未削除 `shopMembers` が存在せず、legacy-onlyのアクセス主体が残っていない。
- 操作者が、対象外のグループに `active` または `readOnly` で所属していない。
- 操作者が、対象外の旧 `shopMembers` に有効な状態で所属していない。
- 外部の有料契約、Subscription準備、支払い方法のSetupを表す課金状態またはprovider資源が存在しない。
- 同じ利用者またはグループに未完了の削除ジョブが存在しない。
- 確認対象のグループIDと、画面に表示したグループ名が現在値と一致する。
...
複数店舗や複数グループに対応した任意店舗の永久削除と、全所属をまとめて閉じるグローバルなアカウント削除は初回の対象外とする。
...
通常の店舗操作と不可逆なグループ終了を同じ行の同格アクションにしない。
...
| 他グループ所属あり | 「ほかのグループにも所属しているため、ログインアカウントを削除できません」と表示する |
...
確認Dialogは `role="alertdialog"` を使い、グループ名の完全一致入力を確定条件にする。
...
- 見出しは「グループとアカウントを削除」とする。
- 本文は「このグループにある店舗を利用できなくなり、スタッフ向けリンクと、まだ送信処理を開始していない通知を停止します。ログインに使っているアカウントも削除します。過去のシフト、請求、監査記録は保持方針に従って保持されます。」とする。
- 入力ラベルは「確認のためグループ名を入力」とする。
- 確定ボタンは「削除手続きを開始」とする。
...
`getEligibility` queryは `organizationId` を受け取り、認証identityからそのグループへの有効な所属を検証したうえで、削除可否、拒否理由のcode、表示用のグループ名と店舗名だけを返す。

他グループのID、管理者一覧、Clerk user ID、ジョブ内部状態は公開DTOへ含めない。
...
既存ジョブの `organizationId` が入力と一致する場合は、利用者、グループ、店舗が削除済みでも同じ `accepted` を返す。

同じ `requestId` で異なるグループを指定した場合は拒否する。

既存ジョブがない場合だけ、削除可否を再計算し、再認証の鮮度を確認してから、次の更新を一つのトランザクションで行う。

1. 同じ利用者またはグループの別の未完了ジョブがないことを確認する。
...
5. 操作者のグループ所属と人物状態を削除済みへ変更する。
6. 課金stateのversionを進め、通知cutoffを設定して古いdeadlineを無効化する。
7. グループ終了の監査イベントを、個人情報を含まないcorrelation ID付きで記録する。
8. 最初のinternal workerをscheduleする。
9. `accepted` と受付IDだけを返す。

親となる利用者、グループ、店舗を先に無効化し、子レコードのbatched cleanup中も新しいアクセスと通知作成を拒否する。
...
- identityから取得した一時的なClerk user IDと、受付時の `expectedClerkIssuer`。
- `status`、table単位の `progress`、`attemptCount`。
- `leaseId`、`leaseExpiresAt`、`nextRunAt`。
- 外部へ返さない安全化済みの `lastErrorCode`。
- `createdAt`、`updatedAt`、`providerDomainVerifiedAt`、`providerUserVerifiedAt`、`deleteAttemptedAt`、`clerkDeletedAt`、`completedAt`。
...
各試行の先頭でClerkのDomain list APIを呼び、返された`frontendApiUrl`のいずれかがジョブ受付時の`expectedClerkIssuer`に一致することを検証する。

Domainとissuerが一致しなければuser APIを呼ばず、`actionRequired: clerk_instance_mismatch` としてfail closedにする。
...
通知は現行Outboxの取消helperを拡張し、対象グループまたは店舗の `pending` と `processing` を `cancelled` にする。

取消理由は既存の `organization_inactive` または `shop_inactive` を再利用する。

`failed` は取消状態として使わない。

送信直前の `prepareForDelivery` でも、削除済みグループまたは店舗を理由に取消する契約を維持する。
...
- `scripts/setupEnv.ts` に既存の `CLERK_SECRET_KEY`、`VITE_CLERK_PUBLISHABLE_KEY`、`CLERK_JWT_ISSUER_DOMAIN` のConvex同期を追加し、全値のstdin入力と秘密を含まない固定エラー整形へ変更する。
...
- `convex/organizationBilling/mutations.ts` と課金actionで削除済みグループへの遅延更新を止める。
...
- `convex/notificationOutbox/mutations.ts` にグループまたは店舗単位の取消処理を追加する。
...
受付完了画面のURLには、受付ID、グループ名、メールアドレスを含めない。
...
- 未認証、権限不足、他グループID、確認名不一致を拒否する。
- Clerk secret、publishable key、issuerのいずれかが未設定なら、ローカル状態を変更せず拒否する。
- 環境変数同期helperが値を子プロセス引数に含めず、失敗時のログにsecret、command、stdout、stderrを出さないことをunit testで確認する。
- 古いDialogからの要求と、店舗切り替え後の要求を拒否する。
- 複数店舗、他管理者、他グループ所属、操作者の対象外legacy所属、対象外の課金状態を拒否する。
...
- 同じ利用者と `requestId` で異なるグループを指定した再送を拒否する。
- 受付トランザクション直後に利用者、グループ、店舗へのアクセスを拒否する。
...
- Secret Keyで取得したClerk domainとissuerが一致しない場合は、`deleteUser` を呼ばずに止める。
...
- 一店舗、一グループ、一管理者の削除要求からClerk削除、cleanup、完了までを通す。
- 二グループ所属の一方だけを対象にした要求がClerk削除へ進まないことを確認する。
...
- 対象外のグループ、店舗、利用者、通知を変更しないことを確認する。
...
実Clerk連携は、使い捨てのClerk利用者と専用グループを作成できるprovider canaryで確認する。
...
2. DevelopのConvex環境へClerkのDevelopment secret、既存publishable key、issuerを同期する。
3. `fva` claim、instance不一致、初回404、検証済み404、再試行、reaperをテスト利用者で確認する。

### PR 2: UIと受付完了画面

1. グループ設定へ危険な操作領域、Dialog、再認証、sign-out、完了画面を追加する。
2. Story、VRT、Behavior Test、controller testを追加する。
3. developで使い捨て利用者のprovider canaryを通す。
4. 機能概要と運用手順を更新する。

### Production公開

1. ProductionのConvex環境へClerkのProduction secret、既存publishable key、issuerを同期し、Domain list APIの`frontendApiUrl`とissuerが一致することを確認する。
2. Production環境の変更と実Clerkユーザー削除について明示的な承認を得る。
3. 監視担当がいる時間帯に、使い捨てのproduction利用者で一件だけcanaryを実行する。
4. 未完了ジョブ、最終エラー、再試行回数、最古の待機時間を確認する。
...
Clerk設定が誤っていたジョブは設定修正後に、Domain list APIと`getUser`の再検証を通る既存のoperator retryだけで再開する。

運用入力からClerk user IDやissuerを直接上書きする修復APIは作らない。

ログにはジョブID、phase、試行回数、安全化したerror codeだけを残す。

Clerk user ID、メールアドレス、JWT、secret、providerの生レスポンスはログへ出さない。

監査には操作者、対象グループ、受付時刻、correlation ID、結果を残すが、復元用の認証情報は残さない。
...
| Tenant boundary | グループ所属、店舗所属、他グループ所属をサーバーで横断確認する |
...
- `organizationId`を持つ現行店舗で削除受付が成功する。
- 複数店舗、他管理者、他グループ所属、対象外課金状態では、Clerk削除へ到達しない。
...
この計画は、初回を「単一グループ、単一店舗、単一管理者の利用終了」に限定する前提である。
...
- [Backend Domain object](https://clerk.com/docs/reference/backend/types/domain)
