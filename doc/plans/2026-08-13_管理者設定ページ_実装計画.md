...
| カード | 補足 | 遷移先 |
|---|---|---|
| 既存スタッフを管理者として招待 | 登録済みのスタッフから選択 | `/settings/managers/invite-staff` |
| 新しいユーザーを管理者として招待 | 新規スタッフを管理者として招待 | `/settings/managers/invite-new` |
...
- 氏名、人物に保存されたシフト通知先メールアドレス、本人なら`あなた`バッジを表示する。
...
- 状態は少なくとも`招待中`、`送信エラー`、`これ以上追加できません`、`エラー`を扱う。`エラー`は内部の`conflict`を表示したもので、集約条件は[現行機能文書](../features/organization-billing.md#管理者招待一覧のエラー)を正とする。`limitReached`は自動再開される保留queueではなく、現在のcapacityでは承認できないというview状態である。
