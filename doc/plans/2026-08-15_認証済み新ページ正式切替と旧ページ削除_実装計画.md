...
> 2026-08-22に、複数組織・複数店舗・複数管理者・支払いのFeature Flagと閉状態を撤去した。  この計画に残る公開範囲・Feature Flagの記述は当時の判断履歴であり、現在のrepository契約は[組織課金、複数店舗、複数管理者](../features/organization-billing.md)、Production反映の証跡は[リリース状態](../manual/release-status.md)を正とする。  認証済みrouteのProduction rollout確認は引き続きこの計画のActive範囲である。
...
二つ目以降の組織に対する2か月のTrial契約は、機能を明示的に有効化した将来経路の内部契約として残す。
