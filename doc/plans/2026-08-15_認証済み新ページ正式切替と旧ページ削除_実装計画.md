...
作成日: 2026-08-15
状態: Active（repository実装とpush前限定検証を完了、commit・PR・GitHub Actions検証待ち）
...
- [ ] `pnpm lint`、`pnpm type-check`、`pnpm test`、`pnpm build`がgreenである。
- [ ] Storybook build、desktop・320px・414px VRT captureがgreenであり、最新head SHAのbaseline compare開始を確認する。compare結果の承認とbaseline更新は行わない。
- [ ] 全通常E2Eと`@e2e-core` gateが、skip、unexpected retry、flakyなしでgreenである。
- [ ] PlaywrightとVRT artifactにtoken、メール、個人情報が残らない。
- [ ] 実施記録へ全コマンド、件数、結果、環境要因を記録する。
- [ ] 目的別commitをpushし、同じhead branchのPull Requestを作成または更新する。
- [ ] 最新head SHAでVRT compareより前のGitHub Actionsがgreenになり、compare開始を確認する。
...
| 未実施 | 最新head SHAの全体gate | GitHub Actions | Pending | commit・push・PR作成後に全test、build、E2E、VRT captureを監視する |
