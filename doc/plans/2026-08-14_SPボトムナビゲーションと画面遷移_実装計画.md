...
作成日: 2026-08-14  
状態: Proposed（画面構成レビューとSPモック作成済み、実装未着手）  
対象: 認証済み管理アプリの情報設計、SPボトムナビゲーション、画面遷移、既存routeの段階移行  
対象外: PCの具体的なレイアウト、公開サイト、スタッフ向けCapability画面のUI変更、backendの保存形式変更

## 1. この計画の位置づけ

この文書は、現在のDashboardと組織設定を、利用者の目的に合わせた5つのトップ画面へ再構成する提案を保存する。
コードにはまだ実装されていないため、現在仕様の正本ではない。

現在の挙動は[機能インデックス](../features/INDEX.md)とコードを正とし、採用後に実装、テスト、対応するFeature Docを更新する。
PCでも同じ目的地と名称を共有するが、この計画で具体的に設計した画面とモックはSPだけである。
...
| スタッフ | 選択組織全体 | 人物を探し、所属店舗と連携状態を確認する | 検索、店舗で絞る、スタッフ追加、人物詳細を開く |
| 対応 | 選択組織全体 | 人の判断や操作を待つ未解決業務を処理する | シフト調整、登録申請確認、通知不達確認 |
| 管理 | 選択組織全体 | 組織、管理者、契約、各店舗を管理する | 組織設定、権限、課金、店舗詳細、店舗追加 |

単一組織や単一店舗では、切り替えられないselectorを表示しない。
複数組織または複数店舗の場合だけ、現在の対象を変える操作を表示する。
...
名前とメールアドレスの検索、店舗filter、人物ごとの所属店舗を初期viewportに置く。
...
画面内の「シフト」「スタッフ」「通知」は、対応項目を絞るcategoryであり、独立したトップ画面ではない。
...
| `/app/actions/registration-requests/:requestId` | スタッフ登録申請の審査 | 非表示 |
| `/app/actions/notification-failures/:failureId` | 通知不達の復旧 | 非表示 |
...
## 14. Dialogから専用ページへ移す作業

URL共有、ブラウザバック、複数段階、長い編集、影響previewを持つ作業は専用ページへ移す。

日常業務の正本ページを導入するPhase 2で、募集作成、スタッフ追加、スタッフ登録申請と通知不達を専用ページへ移す。
シフト調整は既存のシフト表を専用ページとして再利用する。
登録申請と通知不達は、完了後に`/app/actions`の元のcategory、店舗filter、scroll位置へ戻す。
...
badgeには「未解決の対応が3件」のようなaccessible nameを与える。
...
| 組織全体の対応一覧 | 3種類の未解決条件と店舗filterを受ける組織scope query | `active | readOnly`のcanonical organization member | organization / shopFilter / cursor | なし | Convex Function Test |
...
- 登録申請と通知不達を専用ページで処理し、完了後に元の対応一覧状態へ戻る
