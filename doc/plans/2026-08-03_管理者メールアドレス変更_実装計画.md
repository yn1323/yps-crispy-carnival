...
> 2026-08-04以降の判断は[認証画面とアカウント設定](../features/auth-pages.md)へ移管した。
>
> 本文は判断と実装の履歴として残し、全所属のメール同期と不一致復旧UIは撤去対象とする。  以前のClerk EmailAddress削除は、後続判断により直前の旧Primaryだけを対象とするログインメール変更の完了処理として再導入した。  現在仕様は[認証画面とアカウント設定](../features/auth-pages.md)を正とする。
...
| Gmailへの変更 | 変更後のGmailと既存パスワードによる通常のメールログインを受入対象とする。Google OAuthの自動連携は対象外とする |
...
| 旧メールへ戻した後のConvex再同期失敗 | 追加メールを削除せず、旧primaryをConvexへ再同期する必須状態を表示する |
...
| `src/stores/accountEmail.ts` | primary変更から同期・旧メール削除まで、同じuserの画面差し替えを抑止する非PIIの一時状態を保持する |
...
### 11.1 実装結果（2026-08-03）

ローカル実装、Function Test、Frontend Unit Test、Storybook Behavior Test、機能文書、実環境受入手順の更新を完了した。  新しいtable、永続job、schema変更、backfillは追加していない。

次のリポジトリ検証は成功した。

- `pnpm lint`
- `pnpm type-check`
- `pnpm test`（全4 project成功。追加した応答喪失testの再実行後はLogic 1008件、Convex Logic 1539件、Convex Scenario 91件、UI 645件）
- `pnpm build`
- `pnpm docs:check`

VRTは計画どおりローカルcaptureやbaseline更新を行わず、CIで判定する。  実メール配送、Clerk reverification、同一Clerk Userの維持、新Gmailでの通常メールログイン、旧メール無効化、Production相当データの一致は、[セキュリティ再検証](../manual/security-validation.md)の`ENV-CLERK-02`で未確認である。

ブラウザの既存セッション、実在顧客、Productionデータは確認していない。  したがって、この計画は`History`へ移さず`Active`のままとする。
