...
Progressive Disclosure（段階的開示）の原則に基づき、初見でも迷わない最小限のUIをデフォルトにする。将来のAI自動割当（直近1-2か月の実績ベース）を見据えつつ、今回は手動割当のUX改善に集中する。
...
- **将来のAI自動割当**: 直近1-2か月の実績ベースでAIがシフト案を生成する構想あり。今回の設計はこれと矛盾しない（DailyView=微調整、OverviewView=全体確認のフローが共通）
