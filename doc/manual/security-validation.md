...
- 初回Setupは所属0件の本人だけに1組織、1店舗、管理者本人、3か月のTrialを作り、二重実行を拒否する。  Trial期限処理は一度だけ予約し、Stripe Customer、Subscription、課金operationを作らない。
...
| `ENV-SETUP-01` | 初回Setup | 専用の新規actorが1組織、1店舗、1管理者、3か月のTrialを作り、再実行が拒否され、Trial期限処理が一度だけ予約され、Stripe Customer、Subscription、課金operationがない |
