...
core E2Eは、エラー系を除く利用者向けのハッピーパスを、機能ごとに一つの契約として持つ。
Pull RequestがそのままリリースへつながるためPRゲートで主要な正常導線の回帰を止める一方、validation、通知の対象集合、DBの細部、境界値は下位層に残す。
Full Regressionを一つのE2E suiteとして表現せず、安定した契約IDを各主担当層へ対応付ける。

各契約は、前の機能で作った結果を次の機能で使う受け渡しを少なくとも一つ画面で確認する。
開始状態はseedで作り、契約どうしを実行順序やデータで依存させない。
長い通しテストはretryで全体をやり直すことになり、操作数に比例してflakeが積み重なるため、PRゲートへ置かない。

外部providerの本人確認を通らないと先へ進めない区間は、アプリへ迂回用の分岐を足さず、provider完了後にアプリが行う内部処理をE2E専用関数から呼ぶ形で代替する。
アプリ側の分岐は本番でも到達し得る認証の抜け道になるためである。
代替した区間はE2Eの保証に含めず、下位層と人間向け運用手順で守る。

通知不達などの失敗状態は、deployment全体へ効くdebug設定ではなく、本番と同じ記録処理を通るseedで対象actorの範囲に作る。
deployment全体の設定は、並列に動く他の契約の状態まで変えてしまうためである。
