...
Dialog表示中のブラウザ戻るは、原則として「閉じる操作とclose lock」の契約に従う。
...
`/manage/billing`のプラン操作Dialogは例外とし、browser historyを追加しない。
このDialogを表示中にブラウザバックを行った場合は、Dialogだけを閉じず、処理中を含めて課金ページから通常どおり離れる。
ページを離れた後に返ったclient側の処理結果でStripeへ遷移せず、server側の冪等性と課金ページ再表示時の未完了Checkout照合によって結果を追跡する。
