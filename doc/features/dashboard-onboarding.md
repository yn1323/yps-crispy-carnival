...
| `api.setup.mutations.setupShopAndManager` | mutation | 組織所属が0件の本人について、最初の1組織、1店舗、管理者本人、シフト連絡先を作成。コード空欄ではPro相当の3か月Trial、有効なコードでは期限なしの支払い不要Pro相当を適用 |
...
- プロモーションコードは6桁の英数字を任意入力とする。  空欄ではPro相当・利用人数上限50名の3か月Trialを作り、Trial期限と課金deadlineを作成する。
- 入力値が前後空白除去・大文字化後にserver-only設定と一致する場合は、Trialに代えて期限なしの支払い不要Pro相当（`complimentary.pro`）を作る。  入力済みのコードが適用できない場合は初回セットアップを拒否し、Trialへfallbackしない。
