...
公開・管理・スタッフ用routeの読み込みや描画に失敗した場合は、「ページを表示できませんでした」と表示し、再読み込み、1〜2時間ほどおいて再ログイン、それでも改善しない場合の問い合わせを順に案内する。  この待ち時間は再試行の目安であり、復旧時間を保証しない。

Providerの初期化やroot documentの描画が失敗した場合にも、Chakraと通常のstylesheetへ依存しない白背景の画面を使う。  ヘッダーとダークモードは持たない。

エラーのmessageは折りたたみ内にテキストとして表示し、利用者が問い合わせ本文へ貼り付けられるようにする。  詳細の開閉はブラウザ標準の`details`、問い合わせは`https://shiftori.app/contact`を別タブで開く通常のリンクを使う。  エラー内容をリンクURLへ付加したり、自動送信したりしない。
