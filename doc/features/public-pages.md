...
3か月無料・クレジットカード登録不要の公開文言は、初回Setupが3か月のTrialを作成するbackend artifactと同時に公開する。  Repository上の契約だけで対象deploymentへの反映を推測せず、初回SetupのTrial、期限処理、Stripeオブジェクト非作成を実環境で確認するまで利用可能とは判定しない。
