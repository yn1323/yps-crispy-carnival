...
`SignupForm`、`public/llms.txt`、記事などの公開コピーが案内する通常経路では、初回Setupの任意のプロモーションコードを空欄にした登録へ、初回登録から3か月の無料トライアルを適用する。
...
3か月無料・クレジットカード登録不要の公開文言は、プロモーションコードが空欄の初回Setupで3か月のTrialを作成するbackend artifactと同時に公開する。  Repository上の契約だけで対象deploymentへの反映を推測せず、空欄経路のTrial、期限処理、Stripeオブジェクト非作成を実環境で確認するまで利用可能とは判定しない。
