...
- シフト表で未保存のユーザー編集がある状態で離脱（アプリ内の戻る・ブラウザバック）しようとすると、確認ダイアログで「保存して離れる」「保存しない」を選ばせる。ダイアログを閉じるとその場に留まる（`useBlocker` + `src/domains/shift/isAssignmentsEqual.ts`）。シフト申請の到着などサーバー由来のデータ変化はdirty扱いにしない。タブを閉じる/リロード時はブラウザ標準の離脱確認のみ表示する。
