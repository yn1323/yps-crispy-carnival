...
| `api.dashboard.queries.getDashboardRecruitments` | query | ダッシュボード初期表示用のシフト候補と提出人数/現在の有効スタッフ数を取得する。削除済み募集と終了済みシフトは返さず、現在・要シフト調整・募集中・未来確定の表示に必要な候補を返す |
| `api.dashboard.queries.hasDashboardPastRecruitments` | query | ダッシュボードで過去のシフト導線を出すため、確定状態にかかわらず終了済みシフトが存在するかを取得する |
| `api.dashboard.queries.getDashboardPastRecruitments` | query | `過去のシフトを見る` 押下後に、確定状態にかかわらず終了済みシフトを終了日が新しい順でページング取得する |
...
- グループは `現在のシフト` → `要シフト調整` → `募集中` → `確定済み` → `過去のシフト` の順に表示し、空グループは見せない。`現在のシフト` は確定済みかつ今日がシフト期間内、`要シフト調整` は未確定・締切済みかつシフト終了日以前、`募集中` は未確定で締切前、`確定済み` は未来の確定済みシフト、`過去のシフト` は確定状態にかかわらずシフト終了日を過ぎたシフトを指す。
  シフト終了日当日は過去扱いにせず、翌日から過去扱いにする。
...
- 状態色は、募集中を緑、要シフト調整をオレンジ、現在または未来の確定済みを青系、終了済みを灰色にする。
  `要シフト調整` バッジでは行動の必要性を示し、締切日は `7/5 締切済み` のように別テキストで残す。
  未確定の過去シフトは灰色の `未確定` バッジと `7/7 期間終了` のような終了日を表示する。
