...
- ダッシュボードのシフト一覧では、確定済みかつ今日がシフト期間内の募集を先頭に表示し、`現在利用中` バッジで示す。現在のシフトは終了日が近い順で、一覧内に重複表示しない。
- シフト一覧のその他の募集はシフト開始日の降順、同じ開始日では作成日時の降順で表示する。状態色は、募集中を緑、締切済み未確定をオレンジ、現在または未来の確定済みを青系、終了済みを灰色にする。
