...
スタッフがメール/LINEのリンクから希望シフトを提出する機能。店舗設定に応じて、時間指定・日付選択・パターン選択のいずれかで入力できる。過去にシフトあり週を提出している場合は、直近の曜日・時間パターンを今回の募集期間へワンクリックで反映できる。
...
募集編集で新しい勤務対象日が加わると、保存済みの希望を保持したまま未提出になる。  
現在の提出済みは提出記録の存在だけでなく、再提出が必要かも含めて判定する。初回提出時刻は保持し、再提出成功時に現在の提出状態と最終提出時刻を更新する。

未提出に戻った人も、未確定・シフト開始前なら締切後に提出できる。  
その後は締切後の再提出を受け付けない。入力中に募集条件が変わった画面では提出を止め、再読み込みを案内する。未保存の入力は引き継がず、同じ提出URLから保存済みの希望と最新条件を開ける。

- 前回パターンは提出明細から取得し、新しいテンプレート用テーブルは持たない。時間指定・パターン選択は `shiftSubmissionSlots`、日付選択での提出は `shiftSubmissionDates` を参照する。
...
- `time` は提出方法内の開始/終了時間を入力範囲として使い、`shiftType` は選択されたパターン時間を `shiftSubmissionSlots` に保存する。勤務パターンは同じ日に複数選べるが、同じ日の同じパターンは重複登録しない。
...
- 確定通知メール/LINEと確定シフト閲覧画面は募集作成時点の提出方法スナップショットを使う。`dateOnly` は通知上 `出勤` として表示し、`shiftType` は `遅番（15:00-22:00）` のようにパターン名と時間を表示する。  時間指定の確定閲覧は`shiftAssignments`をread-timeで正規化し、同一スタッフ・同一日・同一ポジションの完全隣接区間を一つの時間帯として返す。  読み込みだけで既存DBの行は書き換えない。
