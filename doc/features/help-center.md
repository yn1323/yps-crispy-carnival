...
タスクカードは同じtaskに属するFAQと使い方の件数を示し、ページ内のタスクセクションへ移動する。
...
`/help`では、FAQをアコーディオンでその場に表示し、使い方は個別ページへのリンクとして表示する。  
検索中は通常のタスク一覧を隠し、FAQと使い方を種類別に表示する。
