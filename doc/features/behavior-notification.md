...
| 6 | 請求先メールアドレス変更通知・管理者招待 | **常にメール**（LINE連携済みでもLINEへ送らない） |

- 宛先メールの正本は`organizationPeople.email`（シフト連絡先）。Clerkのログイン用メールや`users.email`を通知先にしない。
- Stripe発の請求書・領収書・決済関連通知は、組織の請求先メールアドレス（`organizations.billingEmail`）へ送る。Stripe Productionの顧客メール設定と実到着は未確認である。
...
| ○ | ×（ブロック・解除） | − | **メール** |
| × | − | − | **メール** |
| −（請求先メールアドレス変更通知・管理者招待） | − | − | **常にメール**（LINE配送を作らない） |
...
| 組織通知（請求先メールアドレス変更・管理者招待・招待承認完了） | 有効管理者。店舗所属を受信条件にしない | - |

組織から削除された人物は、すべての組織通知から除外する。各送信時点で宛先を再抽出する。

受信者ロール別の早見表（○＝受信対象、×＝対象外）:

| 通知グループ | シフト対象スタッフ | 対象外スタッフ | 店舗所属のactive管理者 | 店舗未所属のactive管理者 | 請求先メール |
|---|:-:|:-:|:-:|:-:|:-:|
| シフト運用（募集・催促・確定・再送） | ○ | × | △（自分がその店舗のシフト対象スタッフでもある場合） | × | × |
| 店舗管理通知（申請digest・確定催促・不達digest・本番募集案内） | × | × | ○ | **×** | × |
| 同意依頼・LINE連携案内 | ○ | ○（シフト以外は届く） | − | − | × |
| 請求先メールアドレス変更通知（シフトリ発） | × | × | ○（有効管理者として） | ○（同左。店舗所属は条件でない） | × |
| 管理者招待 | ×（招待先メールのみ） | − | − | − | × |
| Stripe直送（請求書・領収書・決済関連） | × | × | × | × | ○ |
...
| 組織の削除・受信者の所属（請求先変更・業務通知） | 対象外なら停止 |
| 招待の有効性（管理者招待） | 投入済みでも取消 |

利用上限状態は、未承認の管理者招待を除く実利用人数、未削除店舗数、有効管理者数から送信時に導出する。
請求先メールアドレス変更通知は、上限超過または利用上限評価不能による業務通知の停止対象に含めない。
...
| 募集条件の変更 | 条件を変更して保存したとき | 提出状態に関係なく対象店舗のシフト対象スタッフ全員 | 変更なし／シフト対象外／削除済み／確定後／開始日0:00以降。締切後の配送は許可する | **載る** |
| 提出催促 | 提出期限の**前日17:00**（作成時と募集編集時に予約。予定時刻が操作時点以前なら予約しない） | 再提出待ちを含む**未提出**のシフト対象スタッフのみ | 募集が`open`でない／削除済み／提出済み／提出期限後／**現在の募集の版で送信済み**／募集条件が更新された旧版／予約のない募集 | 載る |
...
募集変更通知は共通のチャネル選択に従い、編集ごとの変更前後と提出期限を伝える。  連続編集でも各変更通知を保持し、同じ通知のretryやメールfallbackでは本文を変えない。明示的な再通知には最新編集の内容を使う。詳細は[募集管理の通知との境界](shift-recruitment-management.md#通知との境界)と[通知配送outbox](notification-outbox.md)を参照する。

提出催促は募集の版ごとに送信済みを判定する。編集後の予定時刻が未来なら再予約し、古い予約・送信待ちの催促は送らない。送信直前にも現在の提出状態と期限を確認する。
...
|---|---|---|---|---|
| シフト確定催促 | 提出期限の**翌日17:00**（作成時と募集編集時に予約） | 対象店舗にスタッフ所属するactive管理者（最大20名） | 発火時に確定済み／削除済み／募集条件が更新された旧版／宛先0人／予定時刻が予約時点で過去 | **載せない**（配送イベントログには残る） |
...
### 4.3 組織向け（請求先変更・招待。すべてメールのみ）

| 通知 | トリガー | 宛先 |
|---|---|---|
| 管理者招待 | 発行・再送 | 招待先メールアドレス |
| 招待承認完了 | 承認 | 承認者本人＋他の全有効管理者 |
| 請求先メールアドレス変更 | 変更操作 | 全有効管理者 |
| 請求書・領収書・決済関連 | Stripeの請求イベント | 組織の請求先メールアドレス（**Stripeが直接送る**） |

- シフトリが送る組織課金メールは「請求通知先メールアドレスを変更しました」だけである。LINE配送を1件も作らず、各送信時点の全有効管理者へ、メールアドレスを相互に見せないよう個別送信する。
- Trial終了、契約開始、プラン変更、解約、支払い失敗、Free移行、再契約では、シフトリの組織課金メールを作らない。請求書、領収書、決済関連通知はStripeへ委ねる。
- 支払い不要Pro相当では請求先メールアドレスを変更できないため、組織課金メールを発生させない。
- Stripe Productionの顧客メール設定と実到着は未確認であり、コードとローカルテストの完了から有効化済みと判断しない。
...
- 上限による停止では業務通知のprovider呼び出しが0件であり、請求先メールアドレス変更通知は停止しないことを対で検証する。
- 請求先メールアドレス変更では全有効管理者の集合、メールだけであること、重複がないことを完全一致で検証する。Trial終了、契約開始、プラン変更、解約、支払い失敗、Free移行、再契約では、組織課金のOutboxと予約jobが0件であることを検証する。
