...
| 組織通知（課金・管理者招待・招待承認完了） | 有効管理者（移行中の旧`readOnly`宛先を含み得る）。店舗所属を受信条件にしない | - |

組織から削除された人物は、すべての組織通知から除外する。各送信時点で宛先を再抽出する。

受信者ロール別の早見表（○＝受信対象、×＝対象外）:

| 通知グループ | シフト対象スタッフ | 対象外スタッフ | 店舗所属のactive管理者 | 店舗未所属のactive管理者 | 旧`readOnly`互換 | 請求先メール |
|---|:-:|:-:|:-:|:-:|:-:|:-:|
...
| 猶予期限前の再案内 | 猶予終了の**3日前**（未払いの場合） | 有効管理者＋移行中の旧`readOnly`宛先 |
| Free移行・利用状況案内 | Trial未契約終了・解約適用・猶予終了・想定外解約 | 全有効管理者 |
| 支払い復旧完了 | 支払い成功 | 有効管理者＋移行中の旧`readOnly`宛先 |
| 請求先メールアドレス変更 | 変更操作 | 全有効管理者＋移行中の旧`readOnly`宛先 |
