...
- シフトリが送る組織課金メールは「請求先メールアドレスを変更しました」だけである。LINE配送を1件も作らず、各送信時点の全有効管理者へ、メールアドレスを相互に見せないよう個別送信する。
- Trial終了、契約開始、プラン変更、解約、支払い失敗、猶予、Free移行、支払い復旧では、シフトリの組織課金メールを作らない。請求書、領収書、決済関連通知はStripeへ委ねる。
...
- 請求先メールアドレス変更では全有効管理者の集合、メールだけであること、重複がないことを完全一致で検証する。Trial終了、契約開始、プラン変更、解約、支払い失敗、猶予、Free移行、支払い復旧では、組織課金のOutboxと予約jobが0件であることを検証する。
