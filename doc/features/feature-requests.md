...
`submitForOrganization`はclientから渡された`expectedOrganizationId`とoptionalな`shopId`を信用しない。  送信者のcanonicalな組織所属を必ず検証し、店舗付きの場合は店舗の組織一致とactive状態もserverで再検証する。  有料機能のwrite policyも送信時に再確認する。
