...
`submitForOrganization`はclientから渡された`expectedOrganizationId`とoptionalな`shopId`を信用しない。  送信者のcanonicalな組織所属を必ず検証し、店舗付きの場合は店舗の組織一致と未削除であることもserverで再検証する。  有料機能のwrite policyも送信時に再確認する。

旧client用の`submit`は選択中店舗をクライアントから受け取り、`managerMutation`で未削除店舗への所属を検証する互換APIとして維持する。  `/dashboard`と認証済みの組織スコープ画面はbrowser storageの店舗IDを組織や店舗の認可根拠に使わない。
