...
スタッフの追加、承認、変更、シフト対象外、店舗所属変更、削除、再追加について、条件の組み合わせごとの結果を定める。  
...
- 同じ組織に既存人物（`organizationPeople`）がいる場合はその人物を再利用し、**登録済みの名前を正**とする。人物が増えないため利用人数も増えない。
...
| 2 | なし | 利用人数が上限（Business 41人目等） | **確定前に拒否**。一部だけ保存しない。問い合わせ導線を表示 |
...
| 7 | - | account deletion受付済みの人物に一致 | 汎用的な追加不可。person・staff・通知予約を一切変更しない |
| 8 | - | 人物とstaffの対応が一意に解決できない不整合 | 汎用的な追加不可（fail-closed、部分適用なし） |
| 9 | - | 操作者がreadOnly／契約制限中／権限なし | 拒否（スタッフ追加ボタンも理由付きで無効） |

再追加（#5）で**復元しないもの**: 旧staffの表示情報、旧staffのシフト提出と割当、管理者権限、ほかの店舗所属、session、magic link、LINE token、canonical LINE連携（組織から削除されていた場合）。  
...
- 削除済み人物・不整合・account deletion受付済みは、一覧の時点から**削除履歴を示さない汎用的な承認不可状態**として返す。
...
| 差分なし／処理中／閲覧専用／契約制限中 | 「変更する」ボタンを無効化。mutationも拒否 |
...
- active/readOnlyの**管理者も**個別・全店舗のスタッフ所属を解除できる。管理者権限と組織人物は維持する。
...
| 対象のメールが組織の請求先メールアドレスに登録済み | 請求先を変更するまで削除を完了できない |
| 対象が最後の復旧担当者（契約制限中） | 引き継ぎまたは復旧完了まで削除不可 |
