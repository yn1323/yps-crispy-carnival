...
- 利用人数は人物単位で数える。管理者兼スタッフ、複数店舗所属、シフト対象外、店舗所属なしはいずれも組織在籍中なら1名。詳細は[認可・制限](behavior-authorization.md)を参照。
- スタッフ一覧の任意順は組織人物単位で1つだけ持つ。店舗一覧は組織共通順の部分列とし、シフト表と希望シフト入力の勤務開始時刻順には適用しない。詳細は[スタッフの並び順](staff-order.md)を参照。

### staff canonical linkのWiden期間

`staffs.organizationId`と`staffs.organizationPersonId`が両方未設定のrowは、m050で安全な人物linkを確定するまで保存形式だけをWidenして受け入れる。これは利用者向けのroleでも削除状態でもないため、「移行中スタッフ」「削除済み」「アーカイブ済み」のbadgeやlabelは表示しない。m050でlinkできた後は専用表示を挟まず、通常のスタッフとして扱う。実際に`isDeleted`となった履歴を履歴画面で「削除済み」と扱う契約は維持する。

未解決rowは、保存済みsessionからstaff・session・shopを再検証する既存スタッフ操作と、保存済み通知宛先を送信直前に同じstaff・shopへ照合する配送だけを一時的に許可する。人物詳細、店舗所属変更、LINE再連携、magic link・session・LINE tokenの新規発行はcanonical linkを必須とし、両IDの片方だけが欠損するrowも含めてfail closedにする。
...
| 登録済みスタッフの追加 | 追加ダイアログ「組織から探して追加する」（`addOrganizationPersonToShop`） | 即時 |
...
| 9 | - | 操作者が有効管理者ではない、または現在の利用上限では追加不可 | 拒否（スタッフ追加ボタンも理由付きで無効） |
...
| 登録済みスタッフ追加 | 送らない（同意済みの人物を再利用） | **連携済みなら送らない**。未連携なら追加先スタッフへ送る | 受付中募集があれば送る |
...
- メール変更後は、**LINEを受信できないスタッフに限り**、変更後の宛先へ受付中の募集（未削除・`open`・シフト開始前・提出期限以前）を再送する。
...
| シフト関連通知（募集開始・募集条件の変更・提出催促・確定、一括/個別再送/追加・メール変更・LINE follow追送のすべて）から除外 | 再び通知対象 |
...
スタッフ詳細の「所属店舗を変更」（人物側）と店舗詳細の「所属スタッフを変更する」（店舗側）は、同じ変更契約とUIパターンで動く。追加・解除のBadgeは操作対象の行に表示し、解除影響はDialog下部の共通calloutへ1件だけ表示する。
...
| 差分なし／処理中 | 「変更する」ボタンを無効化 |
| 同じ`requestId`＋同じ変更内容の再実行 | 最初に確定した結果を返す（staff・通知・監査を重複させない） |
| 同じ`requestId`を**異なる内容**へ再利用 | 拒否 |

### 5.2 チェックの扱い

- 非削除店舗は所属中・未所属を問わず編集できる。削除済み店舗は表示しない。
- 追加と解除は「変更する」1回で確定し、二重確認Dialogは開かない。追加行は`店舗に追加`、解除行は`店舗から外す`と表示する。解除対象が1件以上ある間だけ、今日以降の割当削除と対象店舗からのシフト通知停止をDialog下部の共通calloutに1件表示する。
...
- **全店舗のチェックを外すことは許可**する。「変更後、このスタッフの所属店舗は0店舗になります」と、組織への所属や利用人数のカウントが残ることを表示する（人数枠を空けるにはユーザー削除）。
- **解除の対象**はactiveな管理者人物でもよい（先に管理者権限を外す必要はなく、管理者権限と組織人物は維持する）。ただし**操作できるのは対象組織のactiveな管理者だけ**である。
...
| 条件 | 結果 |
|---|---|
| 対象がactiveな管理者 | **拒否**。先に管理者設定で権限を外す（mutationがserver-side guardで再確認） |
