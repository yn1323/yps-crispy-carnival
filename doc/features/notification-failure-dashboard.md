...
送信できなかった通知を `notificationFailureInbox` から店舗単位で読み取り、Dashboard の「要対応」から再通知または「再送せず破棄する」を受け付ける機能。再通知は配送完了ではなく、Outbox または再通知 action に載った時点で受付済みとして扱う。
...
- 通知種別が`通知`（`other` = どの通知種別にもマッピングされないcontext）の不達は、管理画面から対応できる対象として扱わず、一覧・要対応有無（HeroSummaryの「送れなかった通知がN件あります」）・日次リマインダーのいずれにも出さない。判定は`isManagerActionableNotificationFailure`（`convex/notificationOutbox/failureResend.ts`）。記録自体は`notificationFailureInbox`に残す（配送ログ・Resend webhook突合のため）。
...
- 再通知受付に成功したカードは右へ退場し、sourceが未解決へ戻った場合は再表示する。
- 「再送せず破棄する」は確認Dialogを経て実行し、成功後は対象カードを一覧から外す。確認文は「この通知は一覧から外れ、再送されません。」とする。
...
- 要対応を開き直すと `status = open` の不達通知だけを表示するため、`retrying` のカードは表示されない。
...
- `LINE連携案内`（context `line.sendInviteEmail`）は募集に紐づかない。不達カードは募集期間を表示対象にせず、スタッフ名、通知種別、店舗、チャネル、検知日時を表示する。
