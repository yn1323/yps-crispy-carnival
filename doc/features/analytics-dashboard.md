...
TOPはタブで `全体サマリー` / `開始前` / `立ち上げ` / `運用中` / `休眠` / `店舗一覧` / `要望` を切り替える。
`全体サマリー` では全店舗数の日次推移、ステージ別店舗数、ステージ別店舗数の日次推移、期間内のステージ遷移率を表示する。
`開始前` はチュートリアルの到達ステップをKPIとして、開始前店舗数、平均経過日数、ドロップアウト起点別店舗数、登録日の新しい順の店舗一覧を表示する。
`立ち上げ` は運用中に進む前に止まりそうな店舗を見るため、立ち上げ店舗数、平均提出率、通知失敗あり店舗数、初回確定済み店舗数、募集数/確定数と初回募集の開始日・締切日・期間日数を含む店舗一覧を表示する。
`運用中` は安定運用中の店舗を比較するため、運用中店舗数、次シフト未設定店舗数、平均スタッフ数、催促送信スタッフ率（現在のシフト対象スタッフのうち催促通知を実送信したスタッフの割合）、確定済みシフトに対する未提出率、締切から確定までの日数、LINE連携率と、これまでのシフト数/募集中のシフト数/平均募集期間/最終確定日を含む店舗一覧を表示する。
`休眠` は復旧候補の把握のため、休眠・離脱疑い店舗数、最終シフトからの日数、最終提出率と、休眠日数/最後のシフト期間/最後の提出率/LINE連携率/最終作成からの日数を含む店舗一覧を表示する。
`店舗一覧` は30日比較とは独立した全店舗ビューとして、登録日時、店舗名、スタッフ数、LINE連携率、現在のステージ、作成シフト数をソート可能な表で表示する。ステージボタンでOR条件の絞り込みを行い、行の詳細から新しい順のシフト履歴（ステータス、期間、締切、提出人数）をモーダルで確認する。
`要望` はログイン後の要望Dialogから届いた内容を、受付日時、店舗、ユーザー種別、要望の列で新しい順に表示する。50件ずつ取得し、`もっと見る`で次のページを読み込む。
...
| `analyticsDashboard/queries:getFeatureRequests` | ログイン後に届いた要望を新しい順にページング |
