...
| `/shops` | 現在の店舗一覧、契約・スタッフ数・直近シフト・最終利用日、要注意の店舗、契約と名称での絞り込み、読み込んだ店舗の並べ替え、日別・期間の指標からの店舗内訳 |
...
| `/notifications` | 通知の状態、期間・組織名・店舗名・店舗・状態・送信方法・種別またはIDによる通知の検索、tokenによるマジックリンクの確認 |
...
前日の数値、日別推移のグラフの日付、日別の表から、その日の実績に含まれる店舗へ移動できます。  
期間内の店舗数からは、期間内に実績がある店舗へ移動できます。期間内に何日実績があっても1店舗として表示し、計測開始後の日がすべて集計済みの場合だけ内訳を返します。  
過去の活動と現在の店舗名は基準時点を区別します。  
...
契約状態のカードを押すと、その契約状態の組織に属する店舗だけを店舗一覧で表示します。カードの区分と店舗一覧の契約の絞り込みは同じです。
...
組織名・店舗名、店舗、状態、送信方法、種別で絞り込めます。店舗の絞り込みは店舗詳細から開きます。通知IDまたはResendのメールIDでは、他の条件なしで1件を探せます。  
組織名・店舗名は、現在の名称の一部と大文字・小文字を区別せずに照合します。削除済みの店舗・組織の旧名称では一致させません。  
...
店舗一覧は全店舗を一度に取得せず、1回に20店舗ずつ登録の新しい順に確認し、「続きを見る」で次のページを読みます。  
名称、契約、要注意の絞り込みはサーバー側で行い、一致する店舗が少ない場合は続きを数回まで自動で読みます。絞り込みはURLに保存します。  
直近シフトは削除されていない募集から選び、未確定・未来の期間も含めて`2026/9/1-9/15`の形式で表示します。  
各列の見出しで、読み込んだ店舗の中で昇順・降順を切り替えられます。スマートフォンでは並べ替える項目と順序を選べます。
...
| `GET` | `/api/analytics/shops` | 店舗一覧、または最大90日の期間の実績の内訳 |
