...
管理者の「LINEで受け取れません」には、公式アカウントのブロック・友だち未追加の可能性と、本人へ再連携リンクを共有するか再連携用メールを送る手順を表示する。  連携結果画面で友だち状態が無効の場合は、友だち追加・ブロック解除の案内と「公式アカウントを開く」ボタンを表示する。リンク先は環境別の`VITE_LINE_OFFICIAL_ACCOUNT_URL`を使い、公開用ビルドでは未設定や不正なURLを拒否する。設定先と確認方法は[LINE通知の設定と運用](../manual/line-notification.md#公開画面の公式アカウントurl)を参照する。
...
同じLINE利用者が明示連携できるactive組織は最大50とし、51組織目の新規連携は既存連携を変更せず拒否する。
