...
- `src/components/features/Line/LineBulkInviteContent/` — 一括連携依頼確認モーダル中身
...
- `src/components/features/Dashboard/StaffRoster/index.tsx` — 上部に「未連携全員に依頼」ボタン
...
| 画面 | 役割 |
|---|---|
| 店長ダッシュボード（既存）| StaffRow メニュー / 一括ボタン経由で連携依頼 |
| LineLinkQrDialog | QR 表示 + URL コピー |
| 連携依頼確認ダイアログ（個別 / 一括） | 送信前の確認 |
...
| `api.line.mutations.sendInviteBulk` | mutation | 未連携全員へ連携依頼メール |
...
- `lineInviteBulk`: 3回/時（shopId キー） — 一括連携依頼メールの連打防止（一括送信は 100ms 間隔で scheduler 分散）
