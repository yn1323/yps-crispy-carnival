...
| 店舗を削除する | その店舗の保存済み順を無効化し、関連データを削除cleanupの対象にする |
...
削除済み人物、削除済みstaff、削除済み店舗、別組織のIDをorder dataから表示へ混入させない。

## 認可と安全性

- 読み取りと保存は、URLや引数の組織IDだけを信用せず、認証中ユーザーのcanonicalな組織所属を確認する。
- 保存は人物ID集合の完全一致とrevisionを再検証し、追加・削除と競合した古い画面からの上書きを拒否する。
- 人物数、未削除店舗数、店舗スタッフ数の既存上限内だけで同期する。上限超過や不整合ではorder stateを無効化し、既存順へ戻す。
...
- `convex/staff/`、`convex/staffRegistration/`、`convex/organization/`：人物・店舗所属・店舗の追加や削除後にorder projectionを同期する書き込み経路。
...
| `api.dashboard.queries.getDashboardStaffs` | `managerQuery` | 未削除店舗のorder projectionが整合する場合は保存済み順でstaffをページングし、それ以外は既存順を返す |
...
| `api.appOrganization.staffOrderMutations.saveOrganizationStaffOrder` | `organizationMutation` | actor、課金状態、人物集合、order fingerprintを再検証し、組織共通順と未削除店舗用projectionを一transactionで保存する |
...
| 新規・再追加を末尾へ置き、店舗所属変更、人物削除、店舗追加・削除、組織削除でも安全に同期または既存順へ戻す | Convex Function / Scenario Test | `convex/staff/mutations.test.ts`、`convex/staffRegistration/mutations.test.ts`、`convex/organization/mutations.test.ts`、`convex/deletionCleanup/mutations.test.ts`、`convex/_scenario/staffOrderLifecycle.test.ts` |
