...
店舗別の操作は、店舗設定カード内の店舗セレクトとURLの`shop`で選択した店舗だけを対象にする。
Hero直下は「ユーザー設定」と「店舗設定」の2カードで構成する。
ユーザー設定カードには、共通プロフィール、管理者権限、グループからの削除をまとめる。
店舗設定カードには、「所属店舗」として表示する未所属を含むグループ内の全店舗、現在店舗のセレクト、選択店舗に対する通知、LINE連携、店舗設定のタブをまとめる。各店舗行から対象店舗のDashboardへ遷移できる。
...
- 管理者権限解除とグループ削除は確認ダイアログから実行し、最後の有効管理者などの制約に違反した場合はサーバーのエラーを表示する。
...
- `src/components/features/UserDetail/`：ユーザー設定と店舗設定の2カード、店舗切り替え、3タブ、スクロールを維持するURL同期、編集と確認操作。
