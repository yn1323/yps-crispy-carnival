...
グループ内の人物を表す`organizationPeople`を正本として、共通プロフィールと管理者権限はユーザー詳細ページ、店舗ごとのスタッフ設定、通知、LINE連携は店舗別設定ページで扱う。
...
店舗別設定ページは`<店舗名>：<ユーザー名>`を見出しとし、LINE連携、通知、シフト対象設定をタブに分けず縦に並べる。
...
ユーザー詳細の`panel`は開いているDialogを表し、`basic`は基本情報、`addShop`は店舗追加を開く。
`panel`を省略したユーザー詳細URLはDialogを開かないページ本体を表す。
...
検索パラメータの`shop`はユーザー詳細へ来たときの選択店舗を表し、`AuthGuard`、`selectedShopAtom`、ヘッダーの店舗選択と同期する。
...
店舗別設定への遷移は通常の履歴を追加し、ブラウザバックで元のユーザー詳細へ戻れるようにする。
店舗別設定の見出しにある戻る操作も、`shop`、`returnTo`、`returnShop`、`returnShopTo`、`users`を維持して元のユーザー詳細へ戻る。
店舗所属解除後は削除済みの店舗別設定を表示し続けず、同じ検索条件のユーザー詳細へ戻る。
ユーザー詳細からの戻る操作は遷移元を`returnTo`で判定する。
...
- 基本情報Dialogと店舗追加Dialogは、PCではモーダル、SPではフルスクリーンで表示する。
...
- 通知、LINE案内、シフト対象設定は個別に処理中状態を表示し、同じ操作の重複送信を防ぐ。
- LINE連携URLの発行中はSkeletonを表示し、成功後は対象ユーザー専用のURLとQRコードをページ内へ表示する。失敗時は既存のエラー通知を表示する。
- 通知対象の募集と確定シフトは、Dashboardと同じ色・状態表現で期間、締切または確定日、提出人数を表示する。
- 管理者権限解除は基本情報Dialog、店舗所属解除は店舗別設定ページの確認Dialog、グループ削除はページ下部の削除カードから開く確認Dialogで実行し、最後の有効管理者などの制約に違反した場合はサーバーのエラーを表示する。
...
- `src/routes/_auth/users.$personId.tsx`：人物IDと`shop`、基本情報・店舗追加の`panel`、戻り先、出発元店舗、一覧表示件数を受け取るURL境界。
...
- `src/components/features/UserDetail/`：基本情報の入口、所属店舗一覧、店舗追加、基本情報Dialog、URL同期、編集と確認操作。
...
| `api.dashboard.queries.getDashboardCurrentRecruitments` | `managerQuery` | `targetShopId`で指定した対象店舗の現在の確定シフトを取得する |
...
| `api.staff.mutations.sendCurrentShiftNotification` | `managerMutation` | `targetShopId`で指定した店舗のスタッフへ現在の確定シフト通知を、actor・グループ単位の再送quota内で予約する |
