...
提出期限は、募集が未確定でシフト開始前かつ変更前の提出期限前に限り、期間・定休日とともに編集できる。
編集時は、変更後の締切を基準に提出催促と管理者確定催促を予約し直し、古い募集条件に対応する催促を無効にする。
...
stateDiagram-v2
    ...
    note right of open
        提出期限の前日17:00 未提出者へ催促（募集の版ごとに1回）
...
| open（開始前・変更前の提出期限前） | 管理者が募集条件を編集（`updateRecruitment`） | 期間・定休日・提出期限を更新し、変更通知と変更後の催促を予約。新しい勤務対象日があれば全員を未提出へ戻す |
...
| open | 提出期限の前日17:00到来 | 再提出待ちを含む未提出のシフト対象スタッフへ催促（募集の版ごとに1回。編集時に`lastReminderSentAt`を解除し、旧版の予約は送らない） |
...
- 募集作成・編集時、催促は**予定時刻が操作時点より未来の場合だけ**予約する（提出期限が近すぎる募集には催促が付かない）。機能デプロイ前から存在し、その後も編集されていない募集へ予約を追加する処理はない。
...
|---|:-:|:-:|:-:|:-:|:-:|
| スタッフ: 初回提出 | ○ | △（確認ダイアログ後） | × | × | × |
| スタッフ: 再提出（上書き） | ○ | △（募集編集で再提出待ちになった人のみ一度可） | × | × | × |
...
| 4 | open | 提出期限後〜シフト開始日0:00前 | **未提出（募集編集による再提出待ちを含む）** | **確認ダイアログを経て一度だけ提出可。提出後は再提出不可** |
...
募集編集で新しい勤務対象日が加わると、提出履歴と残る日の希望を保持したまま再提出待ちになる。  現在の提出済み判定には、この再提出待ちを含めない。古い募集条件で開いた画面からの提出は拒否し、再読み込みを案内する。
...
| 日付選択（`dateOnly`） | 出勤可能日のみ選択 | `shiftSubmissionDates`。時間スロットは作らない。通知上は「出勤」と表示 |
| パターン選択（`shiftType`） | パターンを選択（同じ日に複数パターン可） | パターンの時間を`shiftSubmissionSlots`へ保存。**同じ日の同じパターンは重複登録しない** |
...
URLを直接開いた、sessionが無効、提出recordがない、再提出待ちの場合は成功を表示しない。query失敗は利用不可と区別し、画面内から再試行できる。
...
- SPの編集画面では、上部の期間表示を省き、下書き保存・確定（確定済みは再送）・出力を文字付きメニューにまとめる。メニュー内で、下書き保存は通知しないことと、確定・再送は保存して通知することを説明する。PCは操作ボタンを横並びで表示する。
- 時間指定のSPは、勤務区間0件なら新規追加、1件なら置換、**2件以上はPC版での編集を案内**（全削除は可能）。
...
| **確認事項（確定を拒否しない）** | 希望との食い違いとして表示のみ | 未提出スタッフへの割当、休み希望日への割当、希望時間外、希望していない勤務パターン |
...
- `confirmRecruitment`自身が再確認するのは**募集の存在・編集版・終了日・状態・保存済み割当の休業日**であり、共通validation全体を単独で再実行しない。古い編集版による保存・確定は拒否する。詳細は[シフト表](shift-board.md#保存確定再通知)を参照する。
- 確定通知の対象は「前回の確定通知snapshotと現在の割当を比較して**変更があるスタッフ**」に加えて、「**前回の確定通知がまだ配送されていないスタッフ**」（`undeliveredStaffIds`。割当が変わっていなくても最新内容で通知する）。
  - 時間指定では、区間の分割・統合だけの表現差は再通知しない。snapshotのsignatureが壊れている場合は安全側で再通知する。
...
| # | 状況 | 結果 |
|---|---|---|
| 1 | 確定通知のリンク（view token）から開く | 提出方法snapshotに沿った確定割当・定休日を表示。時間指定は隣接区間を統合して表示 |
