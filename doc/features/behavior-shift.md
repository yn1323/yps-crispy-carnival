...
| 名称 | 定義 | 実装 |
|---|---|---|
| 提出締切の有効期限 | 締切日の**23:59まで有効**（cutoff＝締切翌日0:00） | `getDeadlineCutoff` |
| スタッフ提出催促の予定時刻 | cutoff − 31時間 ＝ **締切前日17:00** | `getReminderScheduledAt` |
| 管理者確定催促の予定時刻 | cutoff ＋ 17時間 ＝ **締切翌日17:00** | `getManagerConfirmationReminderAt` |
| 提出リンクが開ける期限 | **シフト開始日0:00より前**まで |  |
| 「過去のシフト」の始まり | シフト**終了日の翌日**から（終了日当日は過去に含めない） |  |
| 募集期間の上限 | 最大31日 | `RECRUITMENT_PERIOD_DAYS_MAX` |

締切は募集作成後に**編集できない**（募集APIは作成と削除のみ）。催促の再スケジュールは存在しない。
...
stateDiagram-v2
    ...
    confirmed --> confirmed : 再編集＋再送（差分のあるスタッフだけ再通知）
    open --> deleted : 削除
    confirmed --> deleted : 削除（確認あり）
    deleted --> [*]

    note right of open
        締切前日17:00 未提出者へ催促（1回だけ）
        締切翌日17:00 管理者へ確定催促（未確定なら）
    end note
...
| open（終了日前） | 管理者が確定（`confirmRecruitment`） | `confirmed`へ。差分のあるスタッフへ確定通知を予約。**締切前の確定を禁止する条件はない** |
| confirmed（終了日前） | 管理者が再編集→再確定 | 前回通知snapshotとの**差分があるスタッフだけ**再通知 |
| open/confirmed | **シフト終了日を過ぎた** | 保存・確定・再通知を**画面とmutationの両方で拒否**。閲覧のみ |
| open | 締切前日17:00到来 | 未提出のシフト対象スタッフへ催促（1回だけ。`lastReminderSentAt`で再送防止） |
| open | 締切翌日17:00到来 | 対象店舗にスタッフ所属するactive管理者へ確定催促 |
...
- 募集作成時、催促は**予定時刻が作成時点より未来の場合だけ**予約する（締切が近すぎる募集には催促が付かない）。機能デプロイ前から存在する募集にも予約はない。
- Dashboardの分類は「現在のシフト」（今日が期間内の確定済み）→「要シフト調整」（締切超過のopen）→「募集中」→「確定済み（未来）」→「過去のシフト」の順で、空の分類は表示しない。
- 締切を過ぎてもopenの募集は`/app/actions`（要対応）にも表示される。締切判定はserver時刻が正本で、画面を開いたまま締切が到来しても再評価される。
...
| 操作 | 締切前（open） | 締切後〜開始前（open） | 確定後〜終了日 | 終了日翌日以降 | 削除済み |
|---|:-:|:-:|:-:|:-:|:-:|
| スタッフ: 初回提出 | ○ | △（確認ダイアログ後） | × | × | × |
| スタッフ: 再提出（上書き） | ○ | × | × | × | × |
...
| # | 募集の状態 | 現在時刻 | 提出済みか | 結果 |
|---|---|---|---|---|
| 1 | open | 締切日23:59まで | 未提出 | 提出できる |
| 2 | open | 締切日23:59まで | 提出済み | **再提出できる**（上書き） |
| 3 | open | **締切後**〜シフト開始日0:00前 | 提出済み | **閲覧のみ**（再提出不可） |
| 4 | open | 締切後〜シフト開始日0:00前 | **未提出** | **確認ダイアログを経て初回提出のみ可** |
...
- 確定通知の対象は「前回の確定通知snapshotと現在の割当を比較して**変更があるスタッフだけ**」。
...
- 再確定の差分通知は「変更なし→通知0件」「1名だけ変更→1名だけ」「分割⇔統合のみ→0件」の3点セット。
