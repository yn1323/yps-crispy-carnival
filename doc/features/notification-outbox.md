...
- `convex/schema.ts` — `notificationOutbox` / `notificationDeliveryEvents` / `notificationUsage` テーブル定義
...
通知配送の内部調査用ログ。Convex Dashboard で店舗・スタッフ・管理者ユーザー・通知種別・発生時刻・エラー内容を追うために使う。管理画面への表示や再送UIは持たない。
