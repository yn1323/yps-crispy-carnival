...
- `recordDeliveryEvent` は `enqueue_failed` / `enqueue_preparation_failed` かつ `shopId` と `dedupeKey` が分かる場合だけ `open` として upsert する。`retry_scheduled` / `fallback_enqueued` / `worker_failed` は要対応扱いにしない。
