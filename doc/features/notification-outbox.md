...
- `convex/_lib/shiftAssignmentNormalization.ts` — 時間指定の確定通知とsnapshotが使うread-time正規化
...
募集の変更通知は編集ごとに別の通知として扱い、連続編集でも未送信の古い変更通知を取り消さない。  
編集時の変更前後の条件を既存のfanout operationへ保存し、その編集で変更した項目だけをLINE・メール本文の先頭側に表示する。提出期限は変更の有無に関係なく23:59まで表示し、「希望シフトを再確認する」ボタンで同じ提出URLへ案内する。URLでは最新の条件を確認できる。締切後でも未確定・開始前なら送るが、送信前に確定・開始・削除されていれば取り消す。

通常の再試行とLINEからメールへのfallbackは、生成済み本文の差分を保つ。管理者による明示的な失敗再通知は最新の編集条件を使う。変更前を保存していない旧operationでは差分を推測せず、変更案内と現在の提出期限を表示する。保存する条件は期間・定休日・締切だけで、宛先・希望・tokenは含めない。新しい履歴テーブルやbackfillは不要である。

スタッフ・管理者への催促は予約時の募集の版をOutboxとfallback・再試行へ引き継ぐ。  
古い版の催促は送信直前に取り消し、スタッフ催促では現在も未提出かを確認する。外部サービスへの送信開始後の取り消しは保証しない。
...
- 時間指定の新しい確定通知は、同一スタッフ・同一日・同一ポジションの完全隣接assignmentだけをread-timeで一つの時間帯へ統合する。  正の空白、異なるポジション、option付き割当、overlap、不正値は自動統合しない。  この読み込みで既存`shiftAssignments`は書き換えない。
