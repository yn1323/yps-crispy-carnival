...
- `convex/notificationOutbox/mutations.ts` — enqueue / claim / sent / failed / retry と要対応Inbox操作
...
| `internal.notificationOutbox.mutations.recordDeliveryEvent` | internalMutation | enqueue失敗・retry・最終失敗・fallback等の配送イベントを内部調査用に記録 |
...
- 記録対象は `enqueue_failed` / `retry_scheduled` / `final_failed` / `fallback_enqueued` / `worker_failed`。
- `createdAt` と `expiresAt` を保持し、90日後に日次cronで削除する。
- `shopId` / `staffId` / `userId` / `outboxId` / `channel` / `dedupeKey` / `notificationContext` を、分かる範囲で保持する。
...
- `markFailed` が最終失敗した outbox を `open` として upsert する。同じ outbox が再失敗した場合は重複作成せず、最終失敗時刻・エラー・試行回数を更新する。
- `recordDeliveryEvent` は `enqueue_failed` かつ `shopId` と `dedupeKey` が分かる場合だけ `open` として upsert する。`retry_scheduled` / `fallback_enqueued` / `worker_failed` は要対応扱いにしない。
...
- メールHTML、LINE本文、payload全文は複製しない。再送元は常に `notificationOutbox` とし、Inboxは `dedupeKey` / `notificationContext` / 最終エラーなどの要約だけを持つ。
