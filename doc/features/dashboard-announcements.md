...
有事の連絡やメンテナンス案内を、シフト担当者Dashboardに新しい順で表示する機能。全体、組織、店舗、組織の契約プランを表示対象にでき、現在の対象に一致する複数件を同時に表示する。管理UIは持たず、Convex Dashboard から `dashboardAnnouncements` に直接登録して運用する。
