...
有事の連絡やメンテナンス案内を、シフト担当者Dashboardに新しい順で表示する機能。全体、組織、店舗、組織の契約プランを表示対象にでき、現在の対象に一致する複数件を同時に表示する。読んだお知らせは詳細Dialogから非表示にできる。管理UIは持たず、Convex Dashboard から `dashboardAnnouncements` に直接登録して運用する。
...
| 画面 | 役割 |
|---|---|
| シフト担当者ダッシュボード | 公開中かつ現在の対象に一致するお知らせを新しい順で表示し、各行の押下で本文Dialogを開く |
...
- 複数公開されている場合は、`displayDate` 降順、同日内は作成日時降順の候補から、表示対象となるすべてを順序どおり表示する。全体向けと現在の組織・店舗・契約プラン向けが同時に該当する場合も、両方を表示する。対象範囲の狭さは優先度に使わない。
...
- 詳細Dialogの「次回以降表示しない」で、そのお知らせを一覧から除外する。「閉じる」だけでは非表示にしない。全件非表示にするとお知らせ欄を表示しない。footerは「閉じる」を左端、「次回以降表示しない」を右端へ配置する。
- 非表示にしたお知らせIDをLocalStorageへ保存する。同じブラウザでは再読み込みや店舗・アカウントの切り替え後も非表示を保ち、同じIDの本文を更新しても復活させない。別IDで登録した新しいお知らせは表示する。
- 非表示の設定はブラウザ単位で、サーバー上のお知らせや別のブラウザの表示には影響しない。復元UIは設けない。ブラウザの保存データを消去すると再表示され、LocalStorageへ保存できない環境では現在の画面だけ非表示にする。
- 自動の既読判定、予約公開、管理UIは持たない。
