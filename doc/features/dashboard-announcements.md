...
有事の連絡やメンテナンス案内を、シフト担当者Dashboardに新しい順で表示する機能。全体、組織、店舗、組織の契約プランを表示対象にでき、現在の対象に一致する複数件を同時に表示する。読んだお知らせは詳細Dialogから削除できる。管理UIは持たず、Convex Dashboard から `dashboardAnnouncements` に直接登録して運用する。
...
- 詳細Dialogの「削除する」で、そのお知らせを一覧から除外する。「閉じる」だけでは削除しない。全件削除するとお知らせ欄を表示しない。
- 削除済みのお知らせIDをLocalStorageへ保存する。同じブラウザでは再読み込みや店舗・アカウントの切り替え後も非表示を保ち、同じIDの本文を更新しても復活させない。別IDで登録した新しいお知らせは表示する。
- 削除はブラウザ単位で、サーバー上のお知らせや別のブラウザの表示には影響しない。復元UIは設けない。ブラウザの保存データを消去すると再表示され、LocalStorageへ保存できない環境では現在の画面だけ非表示にする。
