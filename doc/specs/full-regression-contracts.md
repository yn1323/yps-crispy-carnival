...
| `E2E-EXPORT-01` | 管理者のシフト表から別タブへ移り、PDF・Excelの実downloadとreloadを接続する | Desktop Chrome / `@shift-export` 独立E2E、通知配送抑止、trace・screenshot・video off | 2026-09-05にローカル `dev:fortunate-mallard-809` で成功。全員非出勤として保存したシフトを検証。既存Viteを使い、再起動・新規サーバー起動なし | `e2e/scenarios/shift-export.test.ts`、`e2e/pages/ShiftExportPage.ts` |
| `E2E-EXPORT-02` | 匿名の出力URL直入力を帳票へ通さず、ログインの復帰先へ保持する | Desktop Chrome / `@shift-export` 独立E2E | 2026-09-05に既存のローカル開発サーバーで成功。ログイン誘導と復帰先の保持を確認 | `e2e/scenarios/shift-export.test.ts` |
