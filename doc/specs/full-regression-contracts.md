...
| `SHIFT-SUBMISSION-01` | P0 | staffが時間指定・日付選択・パターン選択で初回提出・再提出する | valid submit session → 入力 → 全置換保存。提出期限後は未確定・開始前の現在未提出者だけ提出可能（編集による再提出待ちを含む） | submission headerと方式別明細、firstSubmittedAt | Dashboard回答数、ShiftBoard、Analytics | 用途違いsession、他店舗、期間外、定休日、不正な勤務パターン、上限超過、利用上限評価不能を既存提出を保って拒否 | なし。募集通知は別契約 | Convex Scenario | Desktop / Mobile Chrome | 実装済み | `convex/shiftSubmission/mutations.test.ts`、`convex/shiftSubmission/queries.test.ts`、`convex/_scenario/shiftRequestCollection.test.ts`、`e2e/scenarios/first-shift-delivery.test.ts`（`E2E-SHIFT-01`）、`e2e/scenarios/release-support-staff-submit.mobile.test.ts`（`E2E-MOBILE-01`） |
...
| `SHIFT-EXPORT-DATA-01` | P0 | 管理者の画面内容とDashboard順を帳票へ渡す | ShiftBoardの現在値と順序 → 型・対象・上限検証 → タブ間転送 | DB書き込みなし | 出力専用ページ | 他利用者・他組織・他店舗・不正payload・期限切れ・過大データを拒否。保存済み割当へfallbackしない | 保存・確定・通知0件 | Frontend Unit | 並び順と認可はConvex Function | 実装済み | `src/components/features/ShiftExport/transfer.test.tsx`、`convex/shiftBoard/exportOrder.test.ts` |
| `SHIFT-EXPORT-FILE-01` | P1 | 画面の勤務内容をA4横のPDFと編集可能なExcelへ出力する | 共通モデル → 全期間または15日以上で前後半分割・期間ごとにスタッフ縦改ページ → ローカルファイル | browser内のBlobのみ | 保存リンク、プレビュー | 区間ごとの2行時間・定休日・名称・Dashboard順を共通化し、状態ラベルと通知状況を含めないし、文字列をExcel数式に変換しない。分割時は日付と勤務欄を一致させ、前半の全スタッフ分から後半へ進む。Excelは期間ごとにシートを分け、長いパターン名と折り返し分の行高を保持する | なし | Logic | 分割切替の即時反映をBehaviorで補助 | 実装済み | `src/components/features/ShiftExport/script.test.ts`、`src/components/features/ShiftExport/layout.test.ts`、`src/components/features/ShiftExport/excel.test.ts`、`src/components/features/ShiftExport/pdf.test.ts`、`src/components/features/ShiftExport/index.stories.tsx` |
| `SHIFT-EXPORT-LIFECYCLE-01` | P0 | 出力許可のある画面内容を別タブで作成・保存する | 未保存も許可・busyを停止 → タブ間転送・受領 → bare認証page・24時間のタブ内保持 → single-flight生成 → 保存 / 失敗再試行 | URLを再生成・scope変更・終了時に破棄 | 出力導線とfallback保存link | 認可喪失・利用者切替・期限切れ・分割設定変更で古いファイルを公開しない | なし | Frontend Unit | Desktop / MobileをBehaviorで補助 | 実装済み | `src/components/features/ShiftBoard/ShiftBoardPage/useShiftBoardPageController.export.test.tsx`、`src/components/features/ShiftExport/useExportDownload.test.tsx`、`src/pages/shift-export/index.test.tsx`、`src/components/features/ShiftExport/index.stories.tsx` |
...
| `BILLING-CHECKOUT-01` | P0 | active managerが有料プラン価格を確認し、Checkout / Portalを開始する。未完了の支払いまたはTrialの支払い方法登録を再開または明示的に終了する | action context再解決 → Price / Customer / Subscription / livemode確認 → session作成。TrialのSetup Checkoutでは検証済みのプラン、料金、税区分、請求周期と、登録時点では未請求である説明を確定ボタン付近へ表示する → プラン操作Dialogはhistory guardを登録せず、同じ課金ページを表示中なら一度だけ遷移する。ページ離脱後の遅延結果では遷移せず、課金画面の初回表示と`pageshow`復元でSessionを再照合する | billing operation、provider mapping。Trialの取消ではoperationだけを解放しTrialを維持する | 課金画面、Webhook収束 | 他組織、removed所属、対象外state、test/live混在、支払い不要Pro相当、設定不備をprovider到達前に拒否する。別Customer、Price、generation、modeのSessionを再開または取消せず、`complete` Sessionをexpireしない。導入前のretrying operationへ新しいCheckout表示payloadを混ぜない | シフトリの課金メールは0件。Stripeの決済関連通知は外部設定 | Frontend Unit / Convex Function | Desktop / Mobile | 実装済み。実provider / deployment未確認 | `convex/organizationStripe/actions.test.ts`（組織scope Actionを直接検証）、`convex/organizationStripe/config.test.ts`、`src/components/features/OrganizationSettings/controllers.test.tsx`、`src/components/features/OrganizationSettings/BillingSettings/BillingActionDialog.test.tsx`、`src/components/features/OrganizationSettings/BillingSettings/actionAdapter.test.ts`、`src/components/features/OrganizationSettings/PlanAndPaymentSection.stories.tsx` |
...
| `E2E-EXPORT-01` | 未保存の勤務を管理者のシフト表から別タブへ渡し、PDF・Excelの実downloadとreloadを接続する | Desktop Chrome / core E2E（`@shift-export`で対象指定可能）、通知配送抑止、trace・screenshot・video off | 2026-09-05にローカル `dev:fortunate-mallard-809` で成功。旧契約では全員非出勤として保存したシフトを検証。2026-09-08の未保存勤務・元タブ終了・復元契約は検証結果を別記する。既存Viteを使い、再起動・新規サーバー起動なし | `e2e/scenarios/shift-export.test.ts`、`e2e/pages/ShiftExportPage.ts` |
| `E2E-EXPORT-02` | 匿名の出力URL直入力を帳票へ通さず、ログインの復帰先へ保持する | Desktop Chrome / core E2E（`@shift-export`で対象指定可能） | 2026-09-05に既存のローカル開発サーバーで成功。ログイン誘導と復帰先の保持を確認 | `e2e/scenarios/shift-export.test.ts` |
...
2026-09-08時点のpublic query、mutation、actionは116個である。
...
| `organization/mutations` | `addShopForOrganization`、`deleteOrganizationForOrganization`、`deleteShop`、`removeManagerRoleForOrganization`、`removePersonFromOrganization`、`removePersonFromShop`、`updateOrganizationNameForOrganization`、`updatePersonProfile` | `SHOP-LIFECYCLE-01`、`DELETE-SHOP-ORGANIZATION-01`、`DELETE-PERSON-01`、`PERSON-ROLE-01`、`ORG-PROFILE-01`。組織単位の更新はcanonical組織境界で認可する。店舗追加は契約状態と上限を再確認し、削除は`isDeleted: false`から`true`への一方向だけとする |
...
| `organizationBilling/mutations` | `updateBillingEmailForOrganization` | `BILLING-ENTITLEMENT-01`、`BILLING-PLAN-CHANGE-01`。認可、契約状態、上限整理・課金操作の許可、冪等性を副作用前に確認する |
...
| `shop/mutations` | `updateShopSettings` | `SHOP-LIFECYCLE-01`。選択店舗の設定を更新する |
...
| 利用箇所のないpublic API | 一部 | `line.queries.getLinkStatusByShop`、`getQuotaStatus`を削除・internal化・互換維持のどれにするか | client利用箇所とrolling deploy終了条件を確認した変更で整理する |
