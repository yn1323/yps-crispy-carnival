...
| `E2E-EXPORT-01` | 管理者のシフト表から別タブへ移り、PDF・Excelの実downloadとreloadを接続する | Desktop Chrome / core E2E（`@shift-export`で対象指定可能）、通知配送抑止、trace・screenshot・video off | 2026-09-05にローカル `dev:fortunate-mallard-809` で成功。全員非出勤として保存したシフトを検証。既存Viteを使い、再起動・新規サーバー起動なし | `e2e/scenarios/shift-export.test.ts`、`e2e/pages/ShiftExportPage.ts` |
...
| `shiftExport/queries` | `getShiftExportData` | `SHIFT-EXPORT-DATA-01`。認証・組織・店舗境界を検証し、保存済み割当と帳票に必要な情報だけを返す |
