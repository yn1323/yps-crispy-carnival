...
| `SHIFT-EXPORT-FILE-01` | P1 | 保存済みシフトをA4横のPDFと編集可能なExcelへ出力する | 共通モデル → 全31日固定・スタッフ縦改ページ → ローカルファイル | browser内のBlobのみ | 保存リンク、プレビュー | 2行時間・定休日・名称・順番・状態を共通化し、文字列をExcel数式に変換しない。Excelは長い勤務区分名を保持し、折り返し分の行高を確保する | なし | Logic | 非該当 | 実装済み | `src/components/features/ShiftExport/script.test.ts`、`src/components/features/ShiftExport/layout.test.ts`、`src/components/features/ShiftExport/excel.test.ts`、`src/components/features/ShiftExport/pdf.test.ts` |
| `SHIFT-EXPORT-LIFECYCLE-01` | P0 | 出力許可のある保存内容だけを別タブで作成・保存する | dirty / busyを停止 → bare認証page → single-flight生成 → 保存 / 失敗再試行 | URLを再生成・scope変更・終了時に破棄 | 出力導線とfallback保存link | 生成中の認可喪失・data切替で古いファイルを公開しない | なし | Frontend Unit | Desktop / MobileをBehaviorで補助 | 実装済み | `src/components/features/ShiftBoard/ShiftBoardPage/useShiftBoardPageController.export.test.tsx`、`src/components/features/ShiftExport/useExportDownload.test.tsx`、`src/pages/shift-export/index.test.tsx`、`src/components/features/ShiftExport/index.stories.tsx` |
