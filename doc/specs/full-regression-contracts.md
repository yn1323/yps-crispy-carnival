...
| `BILLING-PLAN-CHANGE-01` | P0 | 公開設定を明示した環境でactive managerが有料プラン変更、期間末解約、予約取消を完了する | preview → operation → Stripe確認 → active / scheduled / grace / restrictedへ収束 | operation、subscription snapshot、billing state | entitlement、利用上限、Dashboard | 通常環境は新規変更を副作用前に拒否する。既存予約を減らす安全な取消とWebhook収束は維持する | 状態遷移時のbilling email | Convex Scenario | Desktop / Mobile | 実装済み。通常は未公開 | `convex/organizationStripe/actions.test.ts`、`convex/_scenario/organizationPaidPlanChanges.test.ts` |
