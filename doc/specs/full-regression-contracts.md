...
| `ORG-CREATE-01` | P1 | 公開設定を明示した環境で、既存管理者が別のFree組織を作る | 公開設定有効・作成上限内 → 本人profileだけsnapshot → `active.free`組織作成 | 新組織、最初の店舗、person、manager、staff、Free枠の選択対象 | 店舗選択、組織設定、利用人数 | 通常の閉状態は副作用0。enabled時も別人物・既存シフトを複製せず、同一request、上限、rate limit、日次budgetを再確認 | 初回LINE連携案内、7日後リマインダーを必要条件で予約。Trial期限処理は予約しない | Convex Scenario | Desktop / Mobile | 実装済み。E2E deploymentだけ明示enable | `convex/setup/mutations.test.ts`、`convex/_scenario/organizationCreation.test.ts`、`src/components/features/OrganizationSettings/controllers.test.tsx`、`src/pages/dashboard/index.stories.tsx`、`src/components/features/OrganizationSettings/OrganizationCreation/OrganizationCreationDialog.stories.tsx` |
