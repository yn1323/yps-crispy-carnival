...
| `BILLING-PLAN-CHANGE-01` | P0 | active managerが有料プラン変更、期間末解約、予約取消を完了する | preview → operation → Stripe確認 → active / scheduled / grace / restrictedへ収束 | operation、subscription snapshot、billing state | entitlement、利用上限、Dashboard | 他組織、readOnly、対象外state、test/live混在を副作用前に拒否し、既存予約を減らす安全な取消とWebhook収束を維持する | 状態遷移時のbilling email | Convex Scenario | Desktop / Mobile | 実装済み。実provider / deployment未確認 | `convex/organizationStripe/actions.test.ts`、`convex/_scenario/organizationPaidPlanChanges.test.ts` |
| `BILLING-TRIAL-CANCEL-01` | P0 | active managerがTrialの継続予約を一度だけ取り消す | cancel request → subscription / generation単位のsingle-flight → provider取消 → local収束 / recovery | 一つのbilling operationと安定idempotency key | Trial終了時の契約制限と再契約導線 | 未認証、readOnly、他組織、対象外stateは副作用0 | 必要な状態通知だけ | Convex Function | Desktop / Mobile | 実装済み。実provider / deployment未確認 | `convex/organizationStripe/actions.test.ts` |
...
| `organizationBilling/mutations` | `setFreeSelection`、`updateBillingEmail`、`updateBillingEmailForOrganization` | `BILLING-ENTITLEMENT-01`、`BILLING-PLAN-CHANGE-01`。認可、契約状態、restricted recovery、冪等性を副作用前に確認する |
