...
export const migration = migrations.define({
  ...
  migrateOne: async (ctx, shopMember) => {
    ...
    const emailNormalized = normalizeMigrationEmail(user.emailNormalized ?? user.email);
    ...
    if (
      managerStaff &&
      (managerStaff.organizationId === undefined || managerStaff.organizationPersonId === undefined)
    ) {
      await ctx.db.patch(managerStaff._id, { organizationId, organizationPersonId: personId });
    }

    await resolveOrganizationMigrationConflicts(ctx, { sourceType: "shopMember", sourceId: shopMember._id });
  },
});
