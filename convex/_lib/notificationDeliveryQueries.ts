...
import { loadShopManagerUsers } from "./shopManagerRecipients";
...
export const isNotificationDeliverySuppressedForShop = internalQuery({
  ...
  handler: async (ctx, { shopId }) => {
    ...
    const managers = await loadShopManagerUsers(ctx, shopId, NOTIFICATION_DRY_RUN_MANAGER_SCAN_LIMIT);
    // 判定対象を全件確認できない時にdry-runへ倒すと実運用managerの通知を黙って落とすため、通常配送にする。
    if (managers.candidateLimitExceeded) return false;

    return managers.users.length > 0 && managers.users.every((manager) => isDryRunManagerEmail(manager.email));
  },
});
