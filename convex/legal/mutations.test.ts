...
describe("legal/mutations", () => {
  ...
  it("管理ユーザーの再同意を記録する", async () => {
    ...
    expect(state?.termsConsentVersion).toBe("manager-terms-consent-2026-08-24-2");
    expect(state?.privacyConsentVersion).toBe("manager-privacy-consent-2026-08-13");
    expect(state?.termsDocumentVersion).toBe("manager-terms-doc-2026-08-24-2");
  ...
  it("管理ユーザーの再同意要否は同意要求版で判定する", async () => {
    ...
    const userId = await t.run(async (ctx) => {
      ...
      await ctx.db.insert("legalConsentStates", {
        ...
        userId,
        shopId,
        termsConsentVersion: "manager-terms-consent-2026-08-24-2",
