...
export const updateShop = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.shopId, fieldsToUpdate).catch((e: unknown) => {
      throw new ConvexError({
...
export const deleteShop = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 論理削除
    await ctx.db.patch(args.shopId, { isDeleted: true }).catch((e: unknown) => {
      throw new ConvexError({
...
export const addUserToShop = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const belonging = await ctx.db
      .query("shopUserBelongings")
      .withIndex("by_shop_and_user", (q) => q.eq("shopId", args.shopId).eq("userId", executor._id))
      .filter((q) => q.neq(q.field("isDeleted"), true))
      .first();
    ...
    const existingBelonging = await ctx.db
      .query("shopUserBelongings")
      .withIndex("by_shop_and_user", (q) => q.eq("shopId", args.shopId).eq("userId", args.userId))
      .filter((q) => q.neq(q.field("isDeleted"), true))
      .first();
