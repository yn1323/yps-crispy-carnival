...
- 対象が数十件程度: 使い捨ての `internalMutation` を書いて `npx convex run` で手動実行
- 対象が多い / 再実行・進捗管理が必要: `@convex-dev/migrations` コンポーネントの導入を検討（未インストール）

### デプロイ前チェック

- [ ] dev 環境（`dev-yps-crispy-carnival`）でスキーマデプロイが通ることを確認
- [ ] 既存ドキュメントが新スキーマに合致するか（Convex はデプロイ時に全件バリデートする）
- [ ] バックフィルが必要な場合、migrate スクリプトを先に流す順序を PR に明記
