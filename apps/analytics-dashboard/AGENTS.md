...
- 集計とJSONLは匿名の数値・計測状態に限定する。問い合わせ閲覧だけは氏名を表示でき、メールアドレスは明示的に開いたスタッフ詳細だけに表示できる。
- 問い合わせの個人情報は永続保存、URL、ログ、外部計測へ載せない。応答はno-store、認証切れではメモリ内のデータも破棄する。token、LINE識別子、通知本文、providerの生のエラーは返さない。
- 業務情報は閲覧専用とし、更新は要望のisDeletedだけを専用経路で行う。本人認証、同一origin、JSONと入力上限をサーバー側で検証し、希望するbooleanを保存する。
