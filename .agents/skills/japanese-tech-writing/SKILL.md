...
description: 日本語の技術文書、設計文書、計画、記事、書籍原稿を作成または推敲するとき、段落構成、論証、用語、読み手の負荷、冗長性を整える。コード、ログ、設定値、短いUI文言だけを扱う作業には使わない。
---
...
- Markdownの地の文は一文ごとに改行し、段落の区切りは空行で示す。表、箇条書き、引用、コードは対象外とする。
...
- 結論、事実、原因、操作を直接書く。不要な前置き、一般論、構成の予告、直前の内容を言い換えただけの総括を挟まない。
- 否定を経由しなくても意味が変わらない文は、内容を直接表す述語へ直す。「値が正しくありません」ではなく「値が誤っています」と書く。
- 禁止、不存在、未対応、未確認など、否定を外すと意味が変わる場合は否定形を残す。「この操作は取り消せません」を、無理に肯定形へ変えない。
- 「AではなくB」「単なるAではありません」は、読者が実際に抱きうる誤解や比較対象を訂正するときだけ使う。Bだけで意味が通るならBを直接述べる。たとえば「これは単なる表示上の問題ではありません。認可の問題です」は「これは認可の問題です」と直す。対比が必要なら、読者がAと考える理由またはAが成り立たない理由を示してからBを書く。
- 翻訳調、難しい漢語、意味の広すぎる動詞、その場限りの造語を避け、一般に定着した平易で具体的な語を使う。
- 誰が何をして、何が起きたかが分かる文にする。受動態や名詞化で行為者と動作を隠さず、具体的な主語と動詞で書く。
- 文の長さ、段落の密度、文末は、内容と論理上の役割に応じて変える。同じ型を機械的に反復せず、変化を作るためだけに文を分割したり、逡巡や言い換えを足したりしない。
- 推敲では、事実、条件、数値、固有名詞、専門用語、不確実性を保つ。実際に問題がある表現と構造だけを直す。
...
- 前の段落との論理関係が自明でない場合は、段落の先頭で接続表現を使う（「であれば」「実際」「しかし」「この例自体からも」）。
...
- 読者が実際に立てそうな誤った解釈だけを扱う。訂正が必要な場合は、否定する内容と根拠を具体的に示し、「直接的で自然な文」の対比規則に従う。
...
- 例の節の中でも、その節の問いや帰結に関係しない過剰な詳細だけを省略する。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTPステータス、カバレッジ率など）や、後で参照しない固有名である。
...
- 術語・訳語は、その分野で慣用されている一般用語を選ぶ（プッシュ通知は「配送」ではなく「配信」、など）。既存用語で表せる概念に新しい名称を作らず、意味の近い漢語を感覚で充てない。
...
- 対比を決め台詞として使わない。対比が必要かどうかは「直接的で自然な文」の規則で判断し、軽い補足や評価は括弧書きで添えてよい。
